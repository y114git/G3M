...
class UseWorker(QThread):
    ...
    def __init__(self, record_id: str, file_path: str, target_kind: TargetKind,
                 mods_dir: str, metadata: dict, parent=None):
    ...
    def run(self):
        try:
            if self._target_kind == TargetKind.MOD:
                ...
                self.use_finished.emit(self._record_id, False, False,
    ...
    def _install_via_deltamod(self, content_path: str, gb_metadata: dict) -> bool:
        try:
            ...
            converter = DeltamodConverter(content_path, self._mods_dir, gb_metadata or None)
    ...
    def _install_via_gamebanana_converter(self, gb_metadata: dict) -> bool:
        try:
            ...
            converter = GameBananaConverter(self._file_path, self._mods_dir, gb_metadata)
            result = converter.convert()
            return bool(result)
        except Exception as e:
    ...
    def _install_deltahub_mod(self, content_path: str, gb_metadata: dict) -> bool:
        try:
            ...
            if not mod_config_path:
                logger.warning('UseWorker: No mod_config.json found, treating as needs_manual')
                return False
            ...
            while os.path.exists(target_mod_dir):
                target_mod_dir = os.path.join(self._mods_dir, f'{folder_name}_{counter}')
                counter += 1
            ...
            for item in os.listdir(content_path):
                ...
                if os.path.islink(src):
                    ...
                    if not link_target.startswith(os.path.realpath(content_path) + os.sep):
                        logger.warning('UseWorker: skipping symlink escaping extraction root: %s', src)
                        continue
    ...
    @staticmethod
    def _update_config_key(mod_dir: str, gb_metadata: dict):
        from config.constants import MOD_CONFIG_FILENAME
