...
class ReportBugWorker(QObject):
    ...
    def __init__(
        self, report_text: str, attached_files: list[str], attach_logs: bool, app_state
    ) -> None:
        super().__init__()
    ...
    def run(self):
        try:
            ...
            for i, file_path in enumerate(self.attached_files):
                ...
                logging.info(
                    f"ReportBugWorker: Copied file: {os.path.basename(file_path)}"
                )
                self.progress.emit(
                    20 + int((i + 1) / total_attachments * 20)
            ...
            if self.attach_logs:
                ...
                if os.path.exists(user_data_root):
                    log_files = [
                        ...
                        for root, _, files in os.walk(user_data_root)
                        for file in files
                        if file.endswith(".log")
                    ]
                logging.info(f"ReportBugWorker: Found {len(log_files)} log files")
                for _i, log_path in enumerate(log_files):
                    try:
                        ...
                        logging.warning(
                            f"ReportBugWorker: Failed to copy log {log_path}: {e}"
                        )
                self.progress.emit(60)
            self.zip_path = os.path.join(
                tempfile.gettempdir(), f"deltahub-report-{os.getpid()}.zip"
            )
            ...
            logging.info(
                f"ReportBugWorker: Created ZIP archive: {self.zip_path} ({zip_size} bytes)"
            )
            ...
            with open(self.zip_path, "rb") as f:
                files = {
                    "file": (os.path.basename(self.zip_path), f, "application/zip")
                }
            ...
            if response.status_code == 200:
                ...
                logging.warning(
                    f"ReportBugWorker: Upload failed. Status: {response.status_code}, Response: {error_text}"
                )
    ...
    def _cleanup(self):
        try:
            if self.temp_dir and os.path.exists(self.temp_dir):
                ...
                logging.info(
                    f"ReportBugWorker: Removed temp directory: {self.temp_dir}"
                )
        except Exception as e:
