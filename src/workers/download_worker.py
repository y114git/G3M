...
class DownloadWorker(QThread):
    progress_updated = pyqtSignal(str, int, int, int)  # record_id, pct, bytes_recv, bytes_total
    download_finished = pyqtSignal(str, bool, str, str)  # record_id, success, error, saved_path

    def __init__(self, record_id: str, url: str, target_path: str, parent=None):
    ...
    def run(self):
        try:
            ...
            total_size = 0
            try:
                total_size = int(session.head(self._url, allow_redirects=True, timeout=NETWORK_TIMEOUT_HEAD).headers.get('content-length', 0))
            except Exception:
                pass
            downloaded_ref = [0]
            ...
            def on_progress(pct):
                if not self._cancelled:
                    self.progress_updated.emit(self._record_id, pct, downloaded_ref[0], total_size)
            ...
            download_file(
                session, self._url, self._target_path,
...
class LocalFileCopyWorker(QThread):
    ...
    def __init__(self, record_id: str, source_path: str, target_path: str, parent=None):
        super().__init__(parent)
        self._record_id = record_id
    ...
    def run(self):
        try:
            os.makedirs(os.path.dirname(self._target_path), exist_ok=True)
            with open(self._source_path, 'rb') as src, open(self._target_path, 'wb') as dst:
                while True:
                    if self._cancelled:
                        break
