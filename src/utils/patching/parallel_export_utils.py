...
def perform_parallel_export(merger, mods_to_export: List[Any], mods_to_apply: List[Any], mod_patched_files: Dict[int, str], mod_types: Dict[int, Dict], vanilla_data_win: str, merge_root: str, cache_running_dir: str, chapter_str: str, chapter_id: int, progress_base: int, export_progress: int, progress_callback) -> bool:
    ...
    merger.patching_logger.info(f'[PARALLEL_EXPORT] Starting parallel export for {total_mods} mod(s) using {max_workers} worker(s)')
    throttler = ProgressThrottler(progress_callback, throttle_ms=150, parent=merger)
    ...
    def export_single_mod(mod_info):
        ...
        try:
            with log_lock:
                merger.patching_logger.info(f'[Mod-{mod_number}] [{thread_id}] Starting export for {mod_name}')
            ...
            if not scripts and comparison_file is None:
                with log_lock:
                    merger.patching_logger.info(f'[Mod-{mod_number}] [{thread_id}] Skipping export - already exported')
                return (mod_number, True, mod_name, 'already exported')
            with log_lock:
                merger.patching_logger.info(f'[Mod-{mod_number}] [{thread_id}] Exporting using strategy: {scripts}, comparison: {comparison_file}')
            success = merger._export_mod_assets_optimized(mod_data_win, mod_number, scripts, comparison_file, vanilla_data_win, merge_root, cache_running_dir, chapter_str)
            if success:
                objects_dir = os.path.join(mod_dir, 'Objects')
                if os.path.exists(objects_dir):
                    with log_lock:
                        merger._log_resource_counts(objects_dir, f'[Mod-{mod_number}] [{thread_id}] Export completed')
                return (mod_number, True, mod_name, None)
            with log_lock:
                merger.patching_logger.warning(f'[Mod-{mod_number}] [{thread_id}] Export failed for {mod_name}')
            return (mod_number, False, mod_name, 'export failed')
        except Exception as e:
            with log_lock:
                merger.patching_logger.error(f'[Mod-{mod_number}] [{thread_id}] Exception during export: {e}', exc_info=True)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_mod):
            ...
            try:
                ...
                merger.patching_logger.error(f'[PARALLEL_EXPORT] Exception in export task: {e}', exc_info=True)
                executor.shutdown(wait=True, cancel_futures=False)
                return False
    throttler.flush()
    merger.patching_logger.info(f'[PARALLEL_EXPORT] All {total_mods} mod(s) exported successfully')
    return True


def perform_parallel_filtering(merger, vanilla_hashes: Dict[str, Dict[str, str]], mods_dirs_info: List[tuple], progress_base: int, filter_progress: int, progress_callback) -> Dict[int, Optional[str]]:
    ...
    max_workers = max(1, min(os.cpu_count() - 1, total_mods, 8))
    merger.patching_logger.info(f'[PARALLEL_FILTER] Starting parallel filtering for {total_mods} mod(s) using {max_workers} worker(s)')
    throttler = ProgressThrottler(progress_callback, throttle_ms=150, parent=merger)
        ...
        try:
            ...
            with log_lock:
                if filtered_dir:
                    ...
                    merger.patching_logger.info(f'[Mod-{mod_number}] [{thread_id}] Filtering completed, no unique resources')
            return (mod_number, filtered_dir, mod_name, None)
        except Exception as e:
            with log_lock:
                merger.patching_logger.error(f'[Mod-{mod_number}] [{thread_id}] Exception during filtering: {e}', exc_info=True)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_mod):
            ...
            try:
                ...
                merger.patching_logger.error(f'[PARALLEL_FILTER] Exception in filtering task: {e}', exc_info=True)
                executor.shutdown(wait=True, cancel_futures=False)
                return {}
    throttler.flush()
    merger.patching_logger.info(f'[PARALLEL_FILTER] All {total_mods} mod(s) filtered successfully')
    return results
