...
def get_default_save_path() -> str:
    ...
    paths = {'Windows': os.path.join(os.environ.get('USERPROFILE', ''), 'AppData', 'Local', 'DELTARUNE'),
             'Darwin': os.path.expanduser('~/Library/Application Support/com.tobyfox.deltarune')}
    default_path = os.path.expanduser(
        '~/.steam/steam/steamapps/compatdata/1690940/pfx/drive_c/users/steamuser/Local Settings/Application Data/DELTARUNE')
...
def is_valid_mac_game_path(path: str, skip_data_check: bool, game_type: str) -> bool:
    ...
    if not path.endswith('.app'):
        app_names = ('UNDERTALE.app',) if game_type == 'undertale' else (
            'DELTARUNE.app', 'DELTARUNEdemo.app')
        app_path = next(
    ...
    if not macos_dir.is_dir() or not res_dir.is_dir():
        return False
    try:
        has_executable = any((p.is_file() and os.access(p, os.X_OK)
...
def is_valid_game_path(path: str, skip_data_check: bool = False, game_type: str = 'deltarune') -> bool:
    ...
    executables = ('UNDERTALE.exe', 'UNDERTALE') if game_type == 'undertale' else (
