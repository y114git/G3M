...
def resolve_game_executable(base_dir: str, is_undertale: bool = False, game_type: str = None) -> str | None:
    try:
        ...
        if system == 'Windows':
            search_order = ['windows', 'linux', 'mac']
        elif system == 'Linux':
            search_order = ['linux', 'windows', 'mac']
        elif system == 'Darwin':
            search_order = ['mac', 'linux', 'windows']
        else:
            search_order = ['windows', 'linux', 'mac']
        for platform_key in search_order:
            for game_type in game_types_to_check:
                game_executables = GAME_EXECUTABLES.get(game_type, {})
                exe_names = list(game_executables.get(platform_key, ()))
                if platform_key == 'mac':
...
def find_chapter_resource_dir(base_dir: str, chapter_id: int) -> str | None:
    try:
        if not base_dir:
            return None
        from config.constants import SLOT_ID_DEMO, SLOT_ID_UNDERTALE, SLOT_ID_UNDERTALE_YELLOW, SLOT_ID_PIZZA_TOWER
        ...
        if platform.system() == 'Darwin':
            ...
            if not os.path.isdir(target_base):
                return None
        if chapter_id in (-1, 0, SLOT_ID_DEMO, SLOT_ID_UNDERTALE, SLOT_ID_UNDERTALE_YELLOW, SLOT_ID_PIZZA_TOWER):
            return target_base
