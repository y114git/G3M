...
class DeltamodConverter:
    """
    Конвертирует моды из формата Deltamod в локальный формат DELTAHUB.
    """
    ...
    def _validate_source(self) -> bool:
        """Проверяет наличие необходимых файлов Deltamod."""
    ...
    def _generate_config_json(self) -> Optional[Dict[str, Any]]:
        """Генерирует содержимое config.json на основе _deltamodInfo.json и modding.xml."""
        ...
        if package_id and package_id != 'und.und.und':
            mod_key = package_id.replace('.', '_')  # Точки нежелательны в именах папок
        else:
    ...
    def _generate_files_structure(self, patches: list) -> Dict[str, Any]:
        """Создает объект 'files' для config.json на основе modding.xml."""
        files_structure = {}
        ...
        for patch in patches:
            ...
            # Определение "слота" (главы/демо)
            chapter_key = None
            if 'demo' in to_path:
                chapter_key = 'demo'
            else:
                match = re.search(r'chapter(\d+)', to_path)
                if match:
                    # Deltamod-нумерация глав может отличаться, приводим к формату DELTAHUB (0 - меню)
                    chapter_num = int(match.group(1))
                    if chapter_num > 0:  # Предполагаем, что chapter1 -> '1', chapter2 -> '2' и т.д.
                        chapter_key = str(chapter_num)
                    # Если нужно будет более сложное сопоставление, логика добавляется здесь

            if not chapter_key:
                # Если не удалось определить главу, пропускаем
                continue
            ...
            if patch_type == 'xdelta':
                ...
                # Deltamod не хранит версии файлов, используем версию мода
    ...
    def _process_files(self, target_mod_dir: str) -> None:
        """Копирует файлы мода в новую структуру папок DELTAHUB."""
        if self.modding_xml is None:
            return
