    ...
    if CURRENT_PLATFORM == 'Darwin':
        if game_path.endswith('.app'):
            ...
            for app_name in os.listdir(game_path):
                if app_name.endswith('.app') and os.path.isdir(os.path.join(game_path, app_name)):
                    candidate = os.path.join(game_path, app_name, 'Contents', 'Resources')
...
def get_protected_exe_paths_with_config(base_folder: str, game_path: str, game_def, local_config: dict) -> set[str]:
...
def safe_archive_name(version_name: str) -> str:
    ...
    safe = _SAFE_RE.sub('_', version_name.strip()).strip(' .')[:80].strip(' .') or 'version'
    return f'{safe}.zip'
