...
from services.mod.service import parse_mod_date


def _get_mod_attr(mod, attr, default=None):
    return (
        mod.get(attr, default) if isinstance(mod, dict) else getattr(mod, attr, default)
    )
...
def _get_mod_tags(mod, is_gamebanana: bool = False):
    ...
    if isinstance(raw_tags, str):
        ...
        tags = []
    category = _get_mod_attr(mod, "gamebanana_category") or _get_mod_attr(
        mod, "category"
    )
...
def _contains_nsfw_text(value) -> bool:
    ...
    return any(
        marker in normalized or marker in collapsed
        for marker in MOD_FILTER_NSFW_TEXT_MARKERS
    )


def _is_nsfw_mod(mod):
    if any(
        _get_mod_bool_attr(mod, attr)
        for attr in (
            "is_nsfw",
            "nsfw",
            "_bIsNsfw",
...
def _sort_date_value(mod, sort_type: int) -> int:
    if sort_type == 1:
        date_str = (
            _get_mod_attr(mod, "last_updated")
            or _get_mod_attr(mod, "updated_date")
            or "0"
        )
    else:
...
def filter_and_sort_mods(
    ...
    search_text, hide_banned = (
        ...
        filters.get("hide_banned", True),
    )
    only_gamebanana, status_filter = (
        filters.get("only_gamebanana", False),
        filters.get("status_filter", ["approved", "pending"]),
    )
    ...
    for item in mods_list:
        ...
        if (
            blocklist_service
            and selected_game
            and blocklist_service.is_mod_blocklisted(mod, selected_game)
        ):
            continue
        if selected_tags:
    ...
    if sort_config:
        sort_type, reverse = (
            sort_config.get("sort_type", 0),
            sort_config.get("reverse", False),
        )
