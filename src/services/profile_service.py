...
PROFILE_STATIC_KEYS = frozenset({
    'selected_game_type', 'chapter_mode_enabled', 'full_install_enabled', 'direct_launch_chapter',
})
...
class ProfileService(QObject):
    ...
    def __init__(self, app_state, settings_service, parent=None):
    ...
    def initialize(self):
        ...
        self._active_name = self.app_state.local_config.get('active_profile', DEFAULT_PROFILE)
    ...
    def save_settings_only(self):
        ...
        data = {k: v for k, v in self.app_state.local_config.items() if not is_profile_key(k)}
    ...
    def create(self, name: str) -> bool:
        name = name.strip()
        if not name or not _has_safe_profile_name(name) or self._profile_path(name).exists():
            return False
    ...
    def duplicate(self, source_name: str, new_name: str) -> bool:
        ...
        if not new_name or not _has_safe_profile_name(new_name) or self._profile_path(new_name).exists():
            return False
    ...
    def rename(self, old_name: str, new_name: str) -> bool:
        new_name = new_name.strip()
        if old_name == DEFAULT_PROFILE or not new_name or not _has_safe_profile_name(new_name) or self._profile_path(new_name).exists():
            return False
        ...
        order = self.app_state.local_config.get('profile_order', [])
        self.app_state.local_config['profile_order'] = [new_name if n == old_name else n for n in order]
    ...
    def _count_all_mods(data: Dict[str, Any], prefix: str = 'used_mods_') -> int:
        return sum(
            ...
            for k, v in data.items() if k.startswith(prefix) and isinstance(v, dict)
            for val in v.values() if val
        )
