...
class UpdateChecker(QObject):
    ...
    def check_for_updates(self):
        ...
        if beta_enabled:
            self.feedback_service.update_status(tr('status.beta_updates_enabled'), UI_COLORS['status_warning'])
        try:
            ...
            if not download_url:
                self.feedback_service.update_status(tr('errors.no_build_for_os', platform=current_platform_key), UI_COLORS['status_warning'])
                return
            ...
            if system == 'Darwin' and 'AppTranslocation' in sys.executable:
                ...
                self.feedback_service.update_status(tr('errors.app_translocation_detected'), UI_COLORS['status_error'])
                return
            ...
            if isinstance(e, requests.RequestException):
                self.feedback_service.update_status(tr('errors.update_check_network_error', error=str(e)), UI_COLORS['status_error'])
            else:
                self.feedback_service.update_status(tr('errors.update_check_general_error', error=str(e)), UI_COLORS['status_error'])
    ...
    def _update_worker(self, update_info):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix='deltahub-update-') as tmp_dir:
                ...
                self.feedback_service.update_status(tr('status.unpacking_and_installing'), UI_COLORS['status_warning'])
                system = platform.system()
                ...
                if system == 'Windows':
                    ...
                    if result > 32:
                        ...
                        self.feedback_service.update_status(tr('status.installer_launched_closing'), UI_COLORS['status_success'])
            ...
            self.feedback_service.update_status(tr('errors.update_permission_error'), UI_COLORS['status_error'])
            ...
            self.feedback_service.update_status(tr('errors.update_failed', error=str(e)), UI_COLORS['status_error'])
