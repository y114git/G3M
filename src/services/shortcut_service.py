...
class ShortcutManager(QObject):
    ...
    def __init__(self, app_state: AppState, feedback_service: FeedbackManager, mod_service: ModManager, parent=None):
    ...
    def create_shortcut_flow(self):
        ...
        if not settings:
            self.feedback_service.show_message('warning', 'dialogs.cannot_create_shortcut_title', tr('dialogs.path_not_specified'))
            return
        mod_count = self._count_selected_mods(settings)
        if mod_count > 1:
            self.feedback_service.show_message('warning', 'dialogs.cannot_create_shortcut_title', tr('dialogs.shortcut_multiple_mods_warning'))
            return
        ...
        if settings.get('launch_via_steam'):
            ...
            custom_path = settings.get('custom_executable_path', '') or settings.get('demo_custom_executable_path', '') or settings.get('undertaleyellow_custom_executable_path', '') or settings.get('pizzatower_custom_executable_path', '') or settings.get('sugaryspire_custom_executable_path', '')
        ...
        if self.feedback_service.ask_question('dialogs.create_shortcut_question', 'dialogs.shortcut_create_description', description_text):
    ...
    def _gather_shortcut_settings(self) -> Optional[Dict[str, Any]]:
        ...
        if is_undertale:
            ...
            sugaryspire_game_path = self.app_state.game_mode.get_game_path(self.app_state.local_config) or ''
        settings = {'launcher_version': LAUNCHER_VERSION, 'game_path': self.app_state.game_path, 'demo_game_path': self.app_state.demo_game_path, 'undertale_game_path': undertale_game_path, 'undertaleyellow_game_path': undertaleyellow_game_path, 'pizzatower_game_path': pizzatower_game_path, 'sugaryspire_game_path': sugaryspire_game_path, 'is_demo_mode': is_demo, 'is_chapter_mode': is_chapter_mode, 'is_undertale_mode': is_undertale, 'is_undertaleyellow_mode': is_undertaleyellow, 'is_pizzatower_mode': is_pizzatower, 'is_sugaryspire_mode': is_sugaryspire, 'launch_via_steam': self.parent_widget.launch_via_steam_checkbox.isChecked(), 'use_custom_executable': self.parent_widget.use_custom_executable_checkbox.isChecked(), 'custom_executable_path': self.app_state.local_config.get(FullGameMode().get_custom_exec_config_key(), ''), 'demo_custom_executable_path': self.app_state.local_config.get(DemoGameMode().get_custom_exec_config_key(), ''), 'undertale_custom_executable_path': self.app_state.local_config.get(UndertaleGameMode().get_custom_exec_config_key(), ''), 'undertaleyellow_custom_executable_path': self.app_state.local_config.get(UndertaleYellowGameMode().get_custom_exec_config_key(), ''), 'pizzatower_custom_executable_path': self.app_state.local_config.get(PizzaTowerGameMode().get_custom_exec_config_key(), ''), 'sugaryspire_custom_executable_path': self.app_state.local_config.get(SugarySpireGameMode().get_custom_exec_config_key(), ''), 'direct_launch_slot_id': self.app_state.local_config.get('direct_launch_slot_id', SLOT_ID_UNIVERSAL), 'mods': {}}
        slot_service = getattr(self.parent_widget, 'slot_service', None) if self.parent_widget else None
        if not slot_service or not hasattr(slot_service, 'get_active_mod_selections'):
            if is_demo:
                ...
                settings['mods']['sugaryspire'] = None
            elif is_chapter_mode:
            ...
            return settings
        if is_demo:
            demo_mods = slot_service.get_used_mods_list(SLOT_ID_DEMO)
            if demo_mods:
            ...
            undertale_mods = slot_service.get_used_mods_list(SLOT_ID_UNDERTALE)
            if undertale_mods:
            ...
            undertaleyellow_mods = slot_service.get_used_mods_list(SLOT_ID_UNDERTALE_YELLOW)
            ...
            sugaryspire_mods = slot_service.get_used_mods_list(SLOT_ID_SUGARY_SPIRE)
            if sugaryspire_mods:
                mod_keys = [get_mod_key(mod) for mod in sugaryspire_mods if get_mod_key(mod)]
                settings['mods']['sugaryspire'] = mod_keys[0] if len(mod_keys) == 1 else mod_keys
            else:
                settings['mods']['sugaryspire'] = None
        elif is_pizzatower:
            from config.constants import SLOT_ID_PIZZA_TOWER
            pizzatower_mods = slot_service.get_used_mods_list(SLOT_ID_PIZZA_TOWER)
            if pizzatower_mods:
            ...
            for chapter_id in chapter_ids:
                mods = slot_service.get_used_mods_list(chapter_id)
                if mods:
            ...
            universal_mods = slot_service.get_used_mods_list(SLOT_ID_UNIVERSAL)
            if universal_mods:
    ...
    def apply_shortcut_mods(self, mods_settings: Dict[str, Any], is_chapter_mode: Optional[bool] = None):
        try:
            ...
            slot_service = getattr(self.parent_widget, 'slot_service', None)
            if not slot_service:
                raise Exception('slot_service not found')
            is_demo = isinstance(self.app_state.game_mode, DemoGameMode)
            is_undertale = isinstance(self.app_state.game_mode, UndertaleGameMode)
            is_undertaleyellow = isinstance(self.app_state.game_mode, UndertaleYellowGameMode)
            is_sugaryspire = isinstance(self.app_state.game_mode, SugarySpireGameMode)
            ...
            if is_demo:
                ...
                if mod_data and mod_data != 'no_change':
                    ...
                    if mods_list:
                        slot_service.set_mods_list(SLOT_ID_DEMO, mods_list, save_state=False)
                    else:
                        slot_service.set_mods_list(SLOT_ID_DEMO, [], save_state=False)
                else:
                    slot_service.set_mods_list(SLOT_ID_DEMO, [], save_state=False)
            elif is_undertale:
                ...
                if mod_data and mod_data != 'no_change':
                    ...
                    mods_list = self._get_mods_from_keys(mod_keys)
                    if mods_list:
                        slot_service.set_mods_list(SLOT_ID_UNDERTALE, mods_list, save_state=False)
                    else:
                        slot_service.set_mods_list(SLOT_ID_UNDERTALE, [], save_state=False)
                else:
                    slot_service.set_mods_list(SLOT_ID_UNDERTALE, [], save_state=False)
            elif is_undertaleyellow:
                from config.constants import SLOT_ID_UNDERTALE_YELLOW
                ...
                if mod_data and mod_data != 'no_change':
                    ...
                    mods_list = self._get_mods_from_keys(mod_keys)
                    if mods_list:
                        slot_service.set_mods_list(SLOT_ID_UNDERTALE_YELLOW, mods_list, save_state=False)
                    else:
                        slot_service.set_mods_list(SLOT_ID_UNDERTALE_YELLOW, [], save_state=False)
                else:
                    slot_service.set_mods_list(SLOT_ID_UNDERTALE_YELLOW, [], save_state=False)
            elif is_sugaryspire:
                mod_data = mods_settings.get('sugaryspire')
                if mod_data and mod_data != 'no_change':
                    mod_keys = mod_data if isinstance(mod_data, list) else [mod_data]
                    mods_list = self._get_mods_from_keys(mod_keys)
                    if mods_list:
                        slot_service.set_mods_list(SLOT_ID_SUGARY_SPIRE, mods_list, save_state=False)
                    else:
                        slot_service.set_mods_list(SLOT_ID_SUGARY_SPIRE, [], save_state=False)
                else:
                    slot_service.set_mods_list(SLOT_ID_SUGARY_SPIRE, [], save_state=False)
            else:
                ...
                if universal_data and universal_data != 'no_change':
                    ...
                    mods_list = self._get_mods_from_keys(mod_keys)
                    if mods_list:
                        slot_service.set_mods_list(SLOT_ID_UNIVERSAL, mods_list, save_state=False)
                    else:
                        slot_service.set_mods_list(SLOT_ID_UNIVERSAL, [], save_state=False)
                elif is_chapter_mode:
                    chapter_ids = [SLOT_ID_MENU, SLOT_ID_CHAPTER_1, SLOT_ID_CHAPTER_2, SLOT_ID_CHAPTER_3, SLOT_ID_CHAPTER_4]
                    for chapter_id in chapter_ids:
                        ...
                        if mod_data and mod_data != 'no_change':
                            ...
                            mods_list = self._get_mods_from_keys(mod_keys)
                            if mods_list:
                                slot_service.set_mods_list(chapter_id, mods_list, save_state=False)
                            else:
                                slot_service.set_mods_list(chapter_id, [], save_state=False)
                        else:
                            slot_service.set_mods_list(chapter_id, [], save_state=False)
                else:
                    slot_service.set_mods_list(SLOT_ID_UNIVERSAL, [], save_state=False)
            logging.info('Shortcut mods applied to used_mods_service')
        ...
        try:
            ...
            if use_custom_executable:
                ...
                if is_demo:
                    ...
                    exec_path = sugaryspire_custom_exec_path
                    if exec_path:
                        self.app_state.local_config[SugarySpireGameMode().get_custom_exec_config_key()] = exec_path
                elif is_pizzatower:
    ...
    def _save_shortcut(self, settings: Dict[str, Any]):
        ...
        try:
            ...
            self.feedback_service.show_message('info', 'dialogs.success', tr('dialogs.shortcut_created_successfully', path=shortcut_path))
            ...
            self.feedback_service.show_message('error', 'errors.error', tr('errors.shortcut_creation_failed', error=str(e)))
