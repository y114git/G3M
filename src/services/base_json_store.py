...
class BaseJsonStore:
    ...
    def __init__(self, base_dir: str, sub_dir: str, filename: str, record_cls) -> None:
    ...
    def load(self) -> list:
        ...
        try:
            with open(self._data_path, encoding="utf-8") as f:
                data = json.load(f)
            self._records = [
                self._record_cls.from_dict(d) for d in data if isinstance(d, dict)
            ]
        except (json.JSONDecodeError, OSError) as e:
            logger.error(
                "%s: corrupt data, backing up and resetting: %s", type(self).__name__, e
            )
