...
class PluginManager(QObject):
    ...
    def __init__(self, app_state, settings_service=None, parent=None) -> None:
        super().__init__(parent)
        self.app_state = app_state
        self.settings_service = settings_service
        self.app_window: Any | None = None
    ...
    def _read_plugins_metadata(self) -> dict[str, Any]:
        ...
        try:
            ...
            self._metadata_cache = (
                load_json(self.app_state.plugins_metadata_path, migrate_config=False)
                or {}
            )
            return self._metadata_cache
    ...
    def _extract_plugin_info_from_file(
        self, plugin_init_py_path: str, plugin_info: dict
    ):
        try:
            import ast

            with open(plugin_init_py_path, encoding="utf-8") as f:
            ...
            strs = data.get(f"LANG_{cur}") or (
                data.get("LANG_EN") if cur != "EN" else None
            )
            if isinstance(strs, dict):
                plugin_id = plugin_info.get("plugin_id") or os.path.basename(
                    os.path.dirname(plugin_init_py_path)
                )
                localization_service.merge_plugin_strings(plugin_id, strs)
        except Exception as e:
            logging.debug(
                f"_extract_plugin_info_from_file: Error extracting info from {plugin_init_py_path}: {e}"
            )
    ...
    def _archive_has_plugin_init(item_path: str, item_name_lower: str) -> bool:
        def _has_init(names):
            return any(
                n.replace("\\", "/").strip("/") == "plugin_init.py"
                or n.replace("\\", "/").strip("/").endswith("/plugin_init.py")
                for n in names
            )
        ...
        try:
            ...
            logging.warning(
                f"convert_plugin_archives: failed to check archive {item_path}: {e}"
            )
        return False
    ...
    def _find_plugin_root(self, temp_dir: str) -> str:
        ...
        if len(contents) == 1:
            ...
            if os.path.isdir(single_dir) and os.path.isfile(
                os.path.join(single_dir, "plugin_init.py")
            ):
                return single_dir
            ...
            for item_name in os.listdir(self.app_state.plugins_dir):
                ...
                try:
                    ...
                    plugin_folder_path = os.path.join(
                        self.app_state.plugins_dir, plugin_folder_name
                    )
                    if os.path.exists(plugin_folder_path):
                        logging.warning(
                            f"convert_plugin_archives: plugin folder already exists: {plugin_folder_name}"
                        )
                        try:
                            os.remove(item_path)
                        except Exception as e:
                            logging.warning(
                                f"convert_plugin_archives: failed to remove archive {item_name}: {e}"
                            )
                        continue
                    try:
                        with tempfile.TemporaryDirectory() as temp_dir:
                            ...
                            for item in os.listdir(source_dir):
                                shutil.move(
                                    os.path.join(source_dir, item), plugin_folder_path
                                )
                        installed_date = datetime.now().strftime("%d.%m.%y %H:%M")
                        self.update_plugin_metadata(
                            plugin_folder_name,
                            {"enabled": True, "installed_date": installed_date},
                        )
                        os.remove(item_path)
                        conversion_happened = True
                        logging.info(
                            f"convert_plugin_archives: extracted plugin archive {item_name} -> {plugin_folder_name}"
                        )
                    except Exception as e:
                        logging.error(
                            f"convert_plugin_archives: Failed to extract {item_name}: {e}",
                            exc_info=True,
                        )
                        with contextlib.suppress(Exception):
                            os.remove(item_path)
                    ...
                    logging.error(
                        f"convert_plugin_archives: Failed to process {item_name}: {e}",
                        exc_info=True,
                    )
    ...
    def load_plugins(self):
        ...
        for plugin_name in os.listdir(self.app_state.plugins_dir):
            ...
            plugin_init_py_path = os.path.join(plugin_path, "plugin_init.py")
            if not os.path.isdir(plugin_path) or not os.path.isfile(
                plugin_init_py_path
            ):
                continue
            if not self.is_plugin_enabled(plugin_name):
            ...
            try:
                spec = importlib.util.spec_from_file_location(
                    f"plugins.{plugin_name}", plugin_init_py_path
                )
                if spec and spec.loader:
                    ...
                    plugin_name_key = (
                        getattr(plugin_module, "PLUGIN_NAME", None) or plugin_id
                    )
                    ...
                    hooks = {
                        "on_before_game_launch": getattr(
                            plugin_module, "on_before_game_launch", None
                        ),
                        "on_after_game_launch": getattr(
                            plugin_module, "on_after_game_launch", None
                        ),
                        "on_before_game_exit": getattr(
                            plugin_module, "on_before_game_exit", None
                        ),
                        "on_after_game_exit": getattr(
                            plugin_module, "on_after_game_exit", None
                        ),
                    }
                    ...
                    is_ui_plugin = (
                        ...
                        and plugin_name_key
                        and (
                            callable(on_tab_open_function)
                            or callable(page_init_function)
                        )
                    )
                    if not is_background_plugin and (not is_ui_plugin):
                        logging.warning(
                            f"Plugin '{plugin_name}' is invalid. It must have at least one hook function or be a UI plugin with PLUGIN_NAME."
                        )
                        self._plugin_errors[plugin_name] = (
                            "Invalid plugin: must have hooks or UI functions"
                        )
                        continue
                    ...
                    if self.app_window:
                        plugin_api = PluginAPI(
                            self.app_state, self.app_window, plugin_id, self
                        )
                        self._plugin_apis[plugin_id] = plugin_api
                    prefixed_name_key = (
                        f"{plugin_id}.{plugin_name_key}" if plugin_name_key else None
                    )
                    plugin_info = {
                        "name": plugin_name,
                        "plugin_id": plugin_id,
                        "name_key": prefixed_name_key,
                        "version": version,
                        "author": author,
                        "description": description,
                        "module": plugin_module,
                        "on_tab_open": on_tab_open_function,
                        "page_init": page_init_function,
                        "tab_hide": tab_hide,
                        "path": plugin_path,
                    ...
                    localized_name = (
                        tr(prefixed_name_key) if prefixed_name_key else plugin_name
                    )
                    logging.info(f"Loaded plugin: {localized_name} (ID: {plugin_id})")
            except Exception as e:
                error_msg = str(e)
                logging.error(
                    f"Failed to load plugin '{plugin_name}': {error_msg}", exc_info=True
                )
                self._plugin_errors[plugin_name] = error_msg
                localized_name = plugin_name
                try:
                    plugin_info_from_file = {}
                    self._extract_plugin_info_from_file(
                        plugin_init_py_path, plugin_info_from_file
                    )
                    plugin_id = plugin_info_from_file.get("plugin_id", plugin_name)
                    plugin_name_key = plugin_info_from_file.get("name_key")
                    if plugin_name_key and (
                        not plugin_name_key.startswith(f"{plugin_id}.")
                    ):
                    ...
                    logging.debug(
                        f"PluginService: failed to localize plugin name for {plugin_id}: {e}",
                        exc_info=True,
                    )
    ...
    def update_plugin_tabs(
        ...
        for plugin in self.app_state.plugins:
            if not plugin.get("tab_hide", False):
                ...
                plugin_tab = (
                    existing_plugin_widgets.get(plugin_name_key)
                    if preserve_widgets
                    else None
                )
                ...
                if plugin_tab is None:
                    ...
                    try:
                        ...
                        logging.debug(
                            f"PluginService: failed to attach plugin metadata for {plugin_name_key}: {e}",
                            exc_info=True,
                        )
                ...
                try:
                    ...
                    logging.debug(
                        f"PluginService: failed to map plugin tab index for {plugin_name_key}: {e}",
                        exc_info=True,
                    )
    ...
    def get_all_plugins_info(self) -> list[dict[str, Any]]:
        ...
        for item_name in os.listdir(self.app_state.plugins_dir):
            ...
            if not installed_date:
                try:
                    if os.path.exists(plugin_init_py_path):
                        ...
                        installed_date = datetime.fromtimestamp(mtime).strftime(
                            "%d.%m.%y %H:%M"
                        )
                        self.update_plugin_metadata(
                            item_name, {"installed_date": installed_date}
                        )
                except Exception as e:
                    logging.debug(
                        f"PluginService: failed to derive installed date for {item_name}: {e}",
                        exc_info=True,
                    )
            plugin_info = {
                "name": item_name,
                ...
                "error": self._plugin_errors.get(item_name),
            }
            ...
            if status == "enabled":
                loaded_plugin = next(
                    (p for p in self.app_state.plugins if p.get("name") == item_name),
                    None,
                )
                if loaded_plugin:
                    for field in (
    ...
    def reload_plugin(self, plugin_name: str):
        plugin_to_reload = next(
            (p for p in self.app_state.plugins if p.get("name") == plugin_name), None
        )
        plugin_id = plugin_to_reload.get("plugin_id") if plugin_to_reload else None
        self.app_state.plugins = [
            p for p in self.app_state.plugins if p.get("name") != plugin_name
        ]
        ...
        for plugin in self.app_state.plugins:
            ...
            if callable(hook_func):
                try:
                    ...
                    localized_name = (
                        tr(name_key) if name_key else plugin.get("name", "Unknown")
                    )
                    logging.info(
                        f"Executing {hook_name} hook for plugin: {localized_name}"
                    )
                    hook_result = hook_func(app_instance)
                    elapsed = time.time() - start_time
                    if elapsed > 1.0:
                        logging.warning(
                            f"Plugin '{localized_name}' hook {hook_name} took {elapsed:.2f}s"
                        )
                    ...
                    logging.error(
                        f"Error executing {hook_name} hook for plugin '{localized_name}': {error_msg}",
                        exc_info=True,
                    )
