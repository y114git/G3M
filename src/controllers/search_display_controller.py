...
class SearchDisplayController(QObject):
    ...
    def _load_more_gamebanana_mods_if_needed(self, items_needed: int | None = None, preferred_game: str | None = None):
        ...
        load_thread = LoadMoreGameBananaModsThread(game_id, start_page, num_pages=1, sort=sort_param, parent=self.app)

        def on_result(mods_list, _id=_identity, _thread=load_thread):
            try:
                current_game_id = GAMEBANANA_GAME_IDS.get(self._get_selected_gamebanana_game())
                current_search = (self.app_state.search_text or '').strip()
                current_sort = self._get_selected_sort()
                current_page = max(1, self.app_state.gamebanana_loaded_pages.get(_id[0], 0) + 1)
                if (current_game_id != _id[0] or current_search != _id[3] or current_sort != _id[2] or current_page != _id[1]):
                ...
                logger.error(f'SearchDisplayController: Error loading more GameBanana mods: {e}', exc_info=True)
                self.app_state.gamebanana_loading = False
                self.update_filtered_mods(preserve_page=True)
    ...
    def _load_search_results_if_needed(self, items_needed: int | None = None, preferred_game: str | None = None):
        if not self.app_state.mods_loaded or self.app_state.gamebanana_loading:
            return
        self._load_more_threads = [t for t in self._load_more_threads if t and t.isRunning()]
        if self._load_more_threads:
        ...
        search_pages = self.app_state.gamebanana_search_loaded_pages.setdefault(search_key, {})
        ...
        self.app_state.gamebanana_loading = True
        self._show_bottom_loading_indicator()
        search_thread = SearchGameBananaModsThread(game_id=game_id, search_string=search_text, start_page=start_page, num_pages=1, sort=_s_sort, parent=self.app)
        ...
        def on_result(mods_list, _id=_s_identity, _thread=search_thread):
            try:
                ...
                current_search = (self.app_state.search_text or '').strip()
                current_sort = self._get_selected_sort()
                current_s_pages = getattr(self.app_state, 'gamebanana_search_loaded_pages', {})
                current_page = max(1, current_s_pages.get(current_search.lower(), {}).get(_id[0], 0) + 1)
                if (current_game_id != _id[0] or current_search != _id[3] or current_sort != _id[2] or current_page != _id[1]):
                    self.app_state.gamebanana_loading = False
                    self._cleanup_load_thread(_thread)
                    return
                ...
                logger.error(f'SearchDisplayController: Error loading search results: {e}', exc_info=True)
                self.app_state.gamebanana_loading = False
                self.update_filtered_mods(preserve_page=True)
    ...
    def _build_filters_and_sort(self):
        ...
        filters = {'tags': selected_tags, 'game': selected_game, 'search_text': self.app_state.search_text, 'hide_banned': True, 'hide_local': True, 'show_nsfw': show_nsfw, 'status_filter': ['approved', 'pending'], 'exclude_installed': False}
        return (filters, None)
    ...
    def _do_update_display(self):
        ...
        try:
            ...
            if app_instance and current_thread != app_instance.thread():
                ...
                QMetaObject.invokeMethod(self, 'update_display', Qt.ConnectionType.QueuedConnection)
                self._update_display_in_progress = False
                return
            ...
            def _add_loading_indicator(position: int):
                ...
                self._place_layout_widget(loading_label, position, column_span=self._mod_list_column_count(), alignment=Qt.AlignmentFlag.AlignCenter)
                ...
                def process_batch(batch_start: int):
                    ...
                    for batch_idx in range(batch_start, batch_end):
                        ...
                        try:
                            ...
                            logger.error(f"Error processing card for mod {(mod.name if mod else 'unknown')} at index {idx}: {e}", exc_info=True)
                            continue
                    return batch_end
    ...
    def _show_bottom_loading_indicator(self):
        ...
        for i in range(layout.count()):
            ...
            if item and item.widget() and getattr(item.widget(), 'objectName', lambda: '')() == 'loading_indicator':
                return
        from PyQt6.QtWidgets import QLabel
        loading_label = QLabel(tr('ui.loading_placeholder'))
        loading_label.setObjectName('loading_indicator')
        ...
        for card in self._iter_layout_cards():
            try:
                ...
                if card.updatesEnabled() != in_range:
                    ...
                    if in_range:
                        card.update()  # force repaint when coming back into view
            except Exception:
                pass
    ...
    def _append_unique_gamebanana_mods(self, mods_list):
        ...
        new_mods_to_add = [m for m in mods_list if (k := get_mod_key(m)) and k not in existing_keys]
        if new_mods_to_add:
    ...
    def _clear_current_gamebanana_mods(self):
        if not hasattr(self.app_state, 'all_mods'):
            return
        self.app_state.all_mods = [m for m in self.app_state.all_mods if not m.is_gamebanana_mod()]
    ...
    def update_search_cards(self):
        try:
            ...
            logger.error(f'SearchDisplayController: Error in update_search_cards: {e}', exc_info=True)
