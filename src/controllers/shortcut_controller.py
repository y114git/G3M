...
def _collect_chapter_data(used_mods_service, app_state) -> Optional[Tuple[Dict[str, Optional[str]], Dict]]:
    ...
    if is_chapter_mode and game_mode.is_multi_tab:
        ...
        for tab in game_mode.tabs:
            has_data = mod and hasattr(mod, 'get_chapter_data') and mod.get_chapter_data(tab.tab_id)
...
def _build_shortcut_config(app_state, chapter_mods: Dict[str, Optional[str]]) -> dict:
    ...
    return {
        ...
        'direct_launch_chapter': app_state.local_config.get('direct_launch_chapter', '') if is_chapter_mode else '',
...
def _generate_shortcut_filename(game_mode, chapter_mod_objects: Dict) -> str:
    ...
    mod_part = get_mod_name(selected_mod, 'mod').replace(' ', '_') if selected_mod else 'Vanilla'
...
def _write_shortcut_file(filepath: str, shortcut_config: dict) -> str:
    ...
    script_path = None if getattr(sys, 'frozen', False) else os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'main.py'))
    config_b64 = base64.b64encode(json.dumps(shortcut_config, ensure_ascii=False).encode('utf-8')).decode('ascii')
    system = platform.system()

    if system == 'Windows':
        vbs_cmd = f'"""" & "{exe_path}" & ""' + (f' "" & "{script_path}" & """"' if script_path else '""')
...
class ShortcutDialog(QDialog):
    ...
    def __init__(self, game_mode, chapter_mod_objects: Dict, shortcut_config: dict, parent=None):
    ...
    def _build_summary(self, gm, chapter_objs, cfg) -> str:
        ...
        if cfg.get('chapter_mode') and gm.is_multi_tab:
            for tab in gm.tabs:
                ...
                mod_name = get_mod_name(mod, 'Unknown') if mod else tr('shortcut.vanilla')
                lines.append(f"  {tr(tab.name_key)}: {mod_name}")
        else:
            any_mod = next((m for m in chapter_objs.values() if m), None)
            mod_name = get_mod_name(any_mod, 'Unknown') if any_mod else tr('shortcut.vanilla')
            lines.append(f"{tr('shortcut.dialog_mod')}: {mod_name}")

        direct_ch = cfg.get('direct_launch_chapter', '')
        if direct_ch and '_' in direct_ch and not direct_ch.endswith('_0') and cfg.get('chapter_mode'):
            tab = gm.get_tab(direct_ch)
            lines.append(f"{tr('shortcut.dialog_direct_launch')}: {tr(tab.name_key) if tab else direct_ch}")

        launch = 'Steam' if cfg.get('launch_via_steam') else 'PortProton' if cfg.get('use_portproton') else tr('shortcut.direct')
...
def _validate_shortcut_prerequisites(app_state, has_any_mod: bool) -> Optional[str]:
...
def on_shortcut_button_click(app_state, feedback_service, used_mods_service, parent_widget: QWidget):
    ...
    if result is None:
        feedback_service.show_message('warning', 'common.warning', tr('shortcut.too_many_mods'))
        return
    ...
    if ShortcutDialog(app_state.game_mode, chapter_mod_objects, shortcut_config, parent_widget).exec() != QDialog.DialogCode.Accepted:
        return

    ext = _get_platform_extension()
    default_name = _generate_shortcut_filename(app_state.game_mode, chapter_mod_objects) + ext
    ext_label = {'vbs': 'VBScript', 'command': 'Command', 'sh': 'Shell Script'}.get(ext.lstrip('.'), 'Script')

    filepath, _ = QFileDialog.getSaveFileName(parent_widget, tr('shortcut.select_folder'), default_name, f"{ext_label} (*{ext})")
    if not filepath:
        return
    ...
    try:
        ...
        feedback_service.show_message('info', 'shortcut.dialog_title', tr('shortcut.created', path=filepath))
    except Exception as e:
        logging.error(f'Failed to create shortcut: {e}', exc_info=True)
        feedback_service.show_message('error', 'errors.error', tr('shortcut.creation_failed', error=str(e)))
