...
class PluginDisplayController:
    ...
    def __init__(self, app_state, feedback_service, plugin_service, app_window) -> None:
    ...
    def _get_plugin_info(self, plugin_name: str):
        return next(
            (
                p
                for p in self.plugin_service.get_all_plugins_info()
    ...
    def _install_plugin(self, source: str, source_label: str) -> None:
        try:
            worker = PluginInstallWorker(
                source, self.app_state.plugins_dir, self.plugin_service, self.app
            )
            self._start_plugin_install(worker)
        except Exception as e:
            logging.error(
                f"PluginDisplayController: Error installing plugin from {source_label}: {e}",
                exc_info=True,
            )
            self.feedback_service.show_message(
                "error", "errors.error", tr("plugins.installation_error", error=str(e))
            )
    ...
    def update_display(self):
        ...
        if not all_plugins:
            ...
            show_empty_message_in_layout(
                ...
                tr("plugins.no_plugins_installed"),
        ...
        for plugin_info in all_plugins:
            ...
            if plugin_name not in existing_names:
                plugin_widget = PluginWidget(
                    plugin_info,
                    parent=self.app.plugins_layout.parent(),
                    parent_app=self.app,
                )
                plugin_widget.clicked.connect(
                    lambda name=plugin_name: self._on_plugin_clicked(name)
                )
                plugin_widget.toggle_requested.connect(
                    lambda name=plugin_name: self.on_plugin_toggle(name)
                )
                plugin_widget.delete_requested.connect(
                    lambda name=plugin_name: self.on_plugin_delete(name)
                )
                self._plugin_widgets[plugin_name] = plugin_widget
                self.app.plugins_layout.insertWidget(
                    self.app.plugins_layout.count() - 1, plugin_widget
                )
        logging.info(
            f"PluginDisplayController: Updated display with {len(all_plugins)} plugins"
        )
    ...
    def on_plugin_toggle(self, plugin_name: str):
        try:
            ...
            if not plugin_info:
                self.feedback_service.show_message(
                    "error", "errors.error", tr("plugins.plugin_not_found")
                )
                return
            ...
            if status == "enabled":
                ...
                self.feedback_service.update_status(
                    tr("plugins.plugin_disabled", name=localized_name),
                    UI_COLORS["status_info"],
                )
            else:
                self.plugin_service.enable_plugin(plugin_name)
                self.feedback_service.update_status(
                    tr("plugins.plugin_enabled", name=localized_name),
            ...
            error_msg = str(e)
            logging.error(
                f"PluginDisplayController: Error toggling plugin {plugin_name}: {error_msg}",
                exc_info=True,
            )
            self.feedback_service.show_message(
                "error", "errors.error", tr("plugins.toggle_error", error=error_msg)
            )
    ...
    def on_plugin_delete(self, plugin_name: str):
        try:
            ...
            reply = QMessageBox.question(
                self.app,
                tr("plugins.delete_plugin_title"),
                tr("plugins.delete_plugin_message", name=localized_name),
            ...
            error_msg = str(e)
            logging.error(
                f"PluginDisplayController: Error deleting plugin {plugin_name}: {error_msg}",
                exc_info=True,
            )
            self.feedback_service.show_message(
                "error", "errors.error", tr("plugins.delete_error", error=error_msg)
            )

    def on_search_plugins(self):
        try:
            webbrowser.open(
                "https://github.com/y114git/ylauncherdata/blob/main/PLUGINS.md"
            )
        except Exception as e:
            logging.error(
                f"PluginDisplayController: Error opening plugins page: {e}",
                exc_info=True,
            )
            self.feedback_service.show_message(
                "error", "errors.error", tr("plugins.open_page_error")
            )
    ...
    def relocalize_plugin_widgets(self):
        for _plugin_name, widget in self._plugin_widgets.items():
