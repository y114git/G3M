...
def handle_tab_changed(w, index):
    ...
    if index == 2 and num_original_tabs == 3:
        if hasattr(w, "plugin_display"):
            QTimer.singleShot(0, w.plugin_display.update_display)
        w.previous_tab_index = index
        return
    if index >= num_original_tabs:
        visible_plugins = [
            p for p in w.app_state.plugins if not p.get("tab_hide", False)
        ]
                ...
                try:
                    ...
                    if isinstance(new_widget, QWidget):
                        try:
                            ...
                            logging.debug(
                                f"on_tab_changed: failed to bind plugin metadata for {plugin.get('name_key')}: {e}",
                                exc_info=True,
                            )
                        w.main_tab_widget.removeTab(index)
                        w.main_tab_widget.insertTab(
                            index, new_widget, tr(plugin["name_key"])
                        )
