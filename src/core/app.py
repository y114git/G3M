...
class DeltaHubApp(QWidget):
    ...
    def __init__(self, args: Optional[argparse.Namespace] = None, parent_for_dialogs: Optional[QWidget] = None, initial_url: str | None = None):
        ...
        self.error_signal.connect(
            lambda msg: QMessageBox.critical(self, tr('errors.error'), msg))
        ...
        self.install_from_gb_signal.connect(
            lambda mod: self._install_single_mod(mod, force=True))
        ...
        self.initialization_timer.timeout.connect(
            self._force_finish_initialization)
        ...
        if not self.local_config.get('first_launch_splash_shown', False):
            self.initialization_finished.connect(
                self._handle_first_launch_settings)
    ...
    def handle_one_click_install(self, url: str):
        ...
        if self.is_installing:
            QMessageBox.warning(self, tr('dialogs.install_in_progress_title'), tr(
                'dialogs.install_in_progress_body'))
            return
        ...
        self.url_install_thread.prompt_required.connect(
            self._handle_url_install_prompt)
    ...
    def _ensure_session_manifest(self) -> dict:
        ...
        if not data:
            data = {'backup_files': {}, 'mod_files_to_cleanup': [
            ], 'mod_dirs_to_cleanup': [], 'backup_temp_dir': None, 'direct_launch': None}
    ...
    def _shortcut_launch(self, args):
        try:
            settings_json = base64.b64decode(
                args.shortcut_launch).decode('utf-8')
        ...
        try:
            if settings.get('is_undertale_mode', False):
                ...
                self.game_mode = DemoGameMode() if settings.get(
                    'is_demo_mode', False) else FullGameMode()
            ...
            use_custom_executable = settings.get(
                'use_custom_executable', False)
            custom_exec_path = settings.get('custom_executable_path', '')
            demo_custom_exec_path = settings.get(
                'demo_custom_executable_path', '')
            ...
            self._launch_game_from_shortcut(launch_via_steam=launch_via_steam, use_custom_executable=use_custom_executable,
                                            custom_exec_path=custom_exec_path, demo_custom_exec_path=demo_custom_exec_path, direct_launch_slot_id=direct_launch_slot_id)
        except Exception as e:
    ...
    def _create_shortcut_flow(self):
        ...
        if not settings:
            QMessageBox.warning(self, tr('dialogs.cannot_create_shortcut_title'), tr(
                'dialogs.path_not_specified'))
            return
        description_lines = [tr('dialogs.shortcut_description'), '', tr(
            'dialogs.current_shortcut_settings'), '']
        game_name = tr('ui.undertale_label') if settings.get('is_undertale_mode', False) else tr(
            'ui.deltarunedemo_label') if settings.get('is_demo_mode', False) else tr('ui.deltarune_label')
        description_lines.append(
            f"<b>{tr('ui.mod_type_label')}</b> {game_name}")
        if settings.get('is_demo_mode', False):
            mod_key = settings['mods'].get('demo')
            if mod_key:
                mod_config = self._get_mod_config_by_key(mod_key)
                mod_name = mod_config.get('name', tr('errors.mod_not_found', mod_key=mod_key)) if mod_config else tr(
                    'errors.mod_not_found', mod_key=mod_key)
                description_lines.append(
                    f"<b>{tr('status.mod_label')}</b> {mod_name}")
            else:
                description_lines.append(
                    f"<b>{tr('status.mod_label')}</b> <i>{tr('status.vanilla')}</i>")
            ...
            if is_chapter_mode:
                if direct_launch_slot_id >= 0:
                    chapter_names = {0: tr('chapters.menu'), 1: tr('tabs.chapter_1'), 2: tr(
                        'tabs.chapter_2'), 3: tr('tabs.chapter_3'), 4: tr('tabs.chapter_4')}
                    chapter_name = chapter_names.get(direct_launch_slot_id, tr(
                        'ui.chapter_tab_title', chapter_num=direct_launch_slot_id))
                    description_lines.append(
                        f"<b>{tr('status.direct_launch_label')}</b> {chapter_name}")
                    ...
                    if mod_key:
                        ...
                        mod_name = mod_config.get('name', tr('errors.mod_not_found', mod_key=mod_key)) if mod_config else tr(
                            'errors.mod_not_found', mod_key=mod_key)
                        description_lines.append(
                            f"<b>{tr('status.mod_for_chapter_label', chapter_name=chapter_name)}</b> {mod_name}")
                    else:
                        description_lines.append(
                            f"<b>{tr('status.mod_for_chapter_label', chapter_name=chapter_name)}</b> <i>{tr('status.no_mod')}</i>")
                else:
                    description_lines.append(
                        f"<b>{tr('status.direct_launch_label')}</b> {tr('status.disabled')}")
                    for chapter_id in [0, 1, 2, 3, 4]:
                        ...
                        if mod_key:
                            ...
                            chapter_names = {0: tr('chapters.menu'), 1: tr('tabs.chapter_1'), 2: tr(
                                'tabs.chapter_2'), 3: tr('tabs.chapter_3'), 4: tr('tabs.chapter_4')}
                            chapter_name = chapter_names.get(chapter_id, tr(
                                'ui.chapter_tab_title', chapter_num=chapter_id))
                            description_lines.append(
                                f'<b>{chapter_name}:</b> {mod_name}')
            else:
                uni_key = settings['mods'].get('universal')
                if uni_key:
                    mod_config = self._get_mod_config_by_key(uni_key)
                    mod_name = mod_config.get('name', tr('errors.mod_not_found', mod_key=uni_key)) if mod_config else tr(
                        'errors.mod_not_found', mod_key=uni_key)
                    description_lines.append(
                        f"<b>{tr('status.mod_label')}</b> {mod_name}")
                else:
                    description_lines.append(
                        f"<b>{tr('status.mod_label')}</b> <i>{tr('status.no_mod')}</i>")
        description_lines.append('')
        if settings.get('launch_via_steam'):
            description_lines.append(f"✓ {tr('status.steam_launch')}")
        elif settings.get('use_custom_executable'):
            custom_path = settings.get('custom_executable_path', '') or settings.get(
                'demo_custom_executable_path', '')
            exe_name = os.path.basename(custom_path) if custom_path else '?'
            description_lines.append(
                f"✓ {tr('status.custom_executable_launch', exe_name=exe_name)}")
        else:
                    ...
                    for folder_name in os.listdir(self.mods_dir):
                        ...
                        if os.path.isfile(test_config_path):
                            try:
                                ...
                                logging.warning(
                                    f'Failed reading config {test_config_path}: {e}')
                                continue
                    if config_path:
                    ...
                    try:
                        ...
                        for folder_name in os.listdir(self.mods_dir):
                            folder_path = os.path.join(
                                self.mods_dir, folder_name)
                            test_config_path = os.path.join(
                                folder_path, 'config.json')
                            if os.path.isfile(test_config_path):
                                try:
                                    test_config = self._read_json(
                                        test_config_path)
                            ...
                            if mod_folder_path:
                                if file_key == 'demo':
                                    chapter_folder = os.path.join(
                                        mod_folder_path, 'demo')
                                elif file_key == 'undertale':
                                    chapter_folder = os.path.join(
                                        mod_folder_path, 'undertale')
                                elif file_key in ['0', '1', '2', '3', '4']:
                                    if file_key == '0':
                                        chapter_folder = os.path.join(
                                            mod_folder_path, 'chapter_0')
                                    else:
                                        chapter_folder = os.path.join(
                                    ...
                                    try:
                                        ...
                                        if ch_id == -1:
                                            ...
                                            chapter_folder = os.path.join(
                                                mod_folder_path, f'chapter_{ch_id}')
                                    except ValueError:
                                        continue
                            data_file_url = ''
                            if chapter_files.get('data_file_url') and mod_folder_path:
                                data_file_url = os.path.join(
                                    chapter_folder, chapter_files['data_file_url'])
                                ...
                                for group_key, filenames in chapter_files['extra_files'].items():
                                    for filename in filenames:
                                        ...
                                        extra_files.append(ModExtraFile(
                                            key=group_key, url=file_path, version='1.0.0'))
                            mod_chapter = ModChapterData(description=config_data.get('tagline', ''), data_file_url=data_file_url, data_file_version=chapter_files.get(
                                'data_file_version', (ch_info.get('versions', {}) or {}).get('data', '1.0.0')), extra_files=extra_files)
    ...
    def _create_settings_nav_button(self, text: str, on_click: Callable, style_sheet: str = '', fixed_width: int = 400) -> QPushButton:
        ...
        button.setStyleSheet(
            f'{base_style} {style_sheet}' if style_sheet else base_style)
        if on_click:
    ...
    def _handle_permission_error(self, path: str):
        ...
        QMessageBox.critical(
            self, tr('errors.access_denied'), detailed_message)
    ...
    def init_ui(self):
        self.full_install_checkbox = QCheckBox(
            tr('ui.install_game_files_first'))
        self.full_install_checkbox.stateChanged.connect(
            self._on_toggle_full_install)
        ...
        self.top_refresh_button.setStyleSheet(
            'min-width:40px; max-width:40px; min-height:40px; max-height:40px; padding:0; margin:0;')
        ...
        self.telegram_button.clicked.connect(lambda: webbrowser.open(
            self.global_settings.get('telegram_url', SOCIAL_LINKS['telegram'])))
        ...
        self.discord_button.clicked.connect(lambda: webbrowser.open(
            self.global_settings.get('discord_url', SOCIAL_LINKS['discord'])))
        self.discord_button.setStyleSheet(
            f"color: {UI_COLORS['social_discord']};")
        ...
        self.main_tab_widget.addTab(
            self.manage_mods_tab, tr('ui.mod_management'))
        self.main_tab_widget.addTab(
            self.xdelta_patch_tab, tr('ui.patching_tab'))
        ...
        self.launch_via_steam_checkbox.stateChanged.connect(
            self._on_toggle_steam_launch)
        settings_center_container.addWidget(
            self.launch_via_steam_checkbox, alignment=Qt.AlignmentFlag.AlignHCenter)
        self.use_custom_executable_checkbox = QCheckBox(
            tr('ui.custom_executable'))
        self.use_custom_executable_checkbox.setToolTip(
            "<html><body style='white-space: normal;'>" + tr('tooltips.custom_exe') + '</body></html>')
        self.use_custom_executable_checkbox.stateChanged.connect(
            self._on_toggle_custom_executable)
        settings_center_container.addWidget(
            self.use_custom_executable_checkbox, alignment=Qt.AlignmentFlag.AlignHCenter)
        self.select_custom_executable_button = QPushButton(
            tr('buttons.select_file'))
        self.select_custom_executable_button.setFixedWidth(153)
        self.select_custom_executable_button.clicked.connect(
        ...
        custom_exe_layout.addWidget(
            self.select_custom_executable_button, alignment=Qt.AlignmentFlag.AlignHCenter)
        custom_exe_layout.addWidget(
            self.custom_executable_path_label, alignment=Qt.AlignmentFlag.AlignHCenter)
        settings_center_container.addWidget(
            self.custom_exe_frame, alignment=Qt.AlignmentFlag.AlignHCenter)
        ...
        settings_center_container.addWidget(
            self.change_path_button, alignment=Qt.AlignmentFlag.AlignHCenter)
        ...
        settings_center_container.addWidget(
            self.change_mods_dir_button, alignment=Qt.AlignmentFlag.AlignHCenter)
        self.customization_button = self._create_settings_nav_button(
            tr('ui.launcher_customization'), lambda: self._switch_settings_page(self.settings_customization_page), fixed_width=200)
        self.reset_button = self._create_settings_nav_button(
            tr('buttons.reset_settings'), self._on_reset_settings_click, fixed_width=200)
        ...
        self.disable_background_checkbox = QCheckBox(
            tr('checkboxes.disable_background'))
        self.disable_background_checkbox.stateChanged.connect(
            self._on_toggle_disable_background)
        self.disable_splash_checkbox = QCheckBox(
            tr('checkboxes.disable_splash'))
        self.disable_splash_checkbox.stateChanged.connect(
            self._on_toggle_disable_splash)
        self.change_background_button = QPushButton(
            tr('buttons.change_background'))
        self.change_background_button.clicked.connect(
            self._on_background_button_click)
        settings_customization_layout = QVBoxLayout(
            self.settings_customization_page)
        self.back_button_cust = QPushButton(tr('ui.back_button'))
        self.back_button_cust.clicked.connect(self._go_back)
        settings_customization_layout.addWidget(
            self.back_button_cust, alignment=Qt.AlignmentFlag.AlignLeft)
        settings_customization_layout.addSpacing(15)
        self.change_background_button = QPushButton()
        self.change_background_button.setFixedWidth(400)
        self.change_background_button.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.change_background_button.clicked.connect(
            self._on_background_button_click)
        settings_customization_layout.addWidget(
            self.change_background_button, 0, Qt.AlignmentFlag.AlignHCenter)
        ...
        self.background_music_button = QPushButton(
            self._get_background_music_button_text())
        self.background_music_button.setFixedWidth(275)
        self.background_music_button.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.background_music_button.clicked.connect(
            self._on_background_music_button_click)
        sound_buttons_layout.addWidget(self.background_music_button)
        self.startup_sound_button = QPushButton(
            self._get_startup_sound_button_text())
        self.startup_sound_button.setFixedWidth(275)
        self.startup_sound_button.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.startup_sound_button.clicked.connect(
            self._on_startup_sound_button_click)
        ...
        def create_setting_row(label_text: str) -> tuple[QHBoxLayout, QLineEdit, QPushButton]:
            ...
            reset_btn.setStyleSheet(
                'min-width: 35px; max-width: 35px; padding-left: 0px; padding-right: 0px;')
            reset_btn.setSizePolicy(
                QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
            reset_btn.clicked.connect(
                lambda: (color_display.clear(), self._on_custom_style_edited()))
        ...
        self.color_widgets = {}
        self.color_config = {'background': tr('ui.background_color'), 'button': tr('ui.elements_color'), 'border': tr(
            'ui.border_color'), 'button_hover': tr('ui.hover_color'), 'text': tr('ui.main_text_color'), 'version_text': tr('ui.secondary_text_color')}
        ...
        for key, label in self.color_config.items():
            ...
            btn.clicked.connect(
                lambda _, le=line_edit: pick_color_for_edit(le))
        ...
        settings_customization_layout.addWidget(
            self.theme_button, 0, Qt.AlignmentFlag.AlignHCenter)
        ...
        self.changelog_widget.setSizePolicy(
        ...
        if doc is not None:
            ...
            doc.setDefaultStyleSheet(
                'p { margin-bottom: 0.75em; } ul, ol { margin-left: 1em; } li { margin-bottom: 0.25em; }')
        self.changelog_text_edit.setOpenExternalLinks(True)
        ...
        self.help_widget.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
        ...
        self.changelog_button.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.changelog_button.setStyleSheet(
    ...
    def _create_search_mods_tab(self):
        ...
        self.search_mods_scroll.setStyleSheet(
        ...
        search_bg_color = get_theme_color(
            self.local_config, 'background', '#000000')
        r, g, b = (int(search_bg_color[1:3], 16), int(search_bg_color[3:5], 16), int(
            search_bg_color[5:7], 16)) if search_bg_color.startswith('#') else (0, 0, 0)
    ...
    def _create_library_tab(self):
        ...
        self.game_type_combo.currentIndexChanged.connect(
        ...
        self.chapter_mode_checkbox.stateChanged.connect(
        ...
        saved_game_type = self.local_config.get(
            'selected_game_type', 'deltarune')
        saved_chapter_mode = self.local_config.get(
            'chapter_mode_enabled', False)
        saved_full_install = self.local_config.get(
        ...
        slots_bg_color = get_theme_color(
            self.local_config, 'background', '#000000')
        ...
        self.active_slots_widget.setStyleSheet(
        ...
        self.installed_mods_container.setSizePolicy(
            QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Expanding)
        ...
        installed_mods_label.setStyleSheet(
        ...
        self.installed_mods_container.setStyleSheet(
    ...
    def _create_filters_widget(self):
        ...
        self.sort_combo.addItems([tr('ui.sort_by_downloads'), tr(
            'ui.sort_by_update_date'), tr('ui.sort_by_creation_date')])
        ...
        self.sort_order_btn.setSizePolicy(
            QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        ...
        self.modgame_combo.addItem(
            tr('dropdowns.filter_deltarune'), 'deltarune')
        self.modgame_combo.addItem(
            tr('dropdowns.filter_deltarunedemo'), 'deltarunedemo')
        self.modgame_combo.addItem(
            tr('dropdowns.filter_undertale'), 'undertale')
        self.modgame_combo.currentIndexChanged.connect(
            self._on_modgame_filter_changed)
    ...
    def _create_pagination_widget(self):
        ...
        pagination_layout.addWidget(
            self.prev_page_btn, alignment=Qt.AlignmentFlag.AlignVCenter)
        ...
        pagination_layout.addWidget(
            self.page_label, alignment=Qt.AlignmentFlag.AlignVCenter)
        ...
        pagination_layout.addWidget(
            self.next_page_btn, alignment=Qt.AlignmentFlag.AlignVCenter)
    ...
    def _show_search_dialog(self):
        if self.search_text:
            ...
            text, ok = QInputDialog.getText(
                self, tr('ui.search_mods'), tr('ui.search_in_name_description'))
            if ok and text.strip():
                self.search_text = text.strip()
                self.search_button.setText('↻')
                self.search_button.setToolTip(
                    tr('ui.clear_search_tooltip', search_text=self.search_text))
                self._update_filtered_mods()
    ...
    def _show_chapter_mode_instruction(self):
        ...
        self.installed_mods_layout.insertWidget(
            self.installed_mods_layout.count() - 1, instruction_widget)
    ...
    def _update_slots_display(self):
        ...
        if self.current_mode == 'normal':
            ...
            slot_names = [tr('chapters.menu'), tr('tabs.chapter_1'), tr(
                'tabs.chapter_2'), tr('tabs.chapter_3'), tr('tabs.chapter_4')]
    ...
    def _create_chapter_indicators(self):
        chapter_names = [tr('ui.menu_label'), tr('ui.chapter_1_label'), tr(
            'ui.chapter_2_label'), tr('ui.chapter_3_label'), tr('ui.chapter_4_label')]
        ...
        for i, chapter_name in enumerate(chapter_names):
            ...
            chapter_label.setStyleSheet(
                f'color: {main_text_color}; font-size: 14px; font-weight: bold;')
            status_label = QLabel('?')
            ...
            self.chapter_indicators[i] = {
                'status_label': status_label, 'chapter_label': chapter_label, 'frame': indicator_frame}
            if hasattr(self, 'active_slots_layout'):
        ...
        if mod is None:
            for i in range(5):
                if i in self.chapter_indicators:
                    ...
                    self.chapter_indicators[i]['status_label'].setStyleSheet(
                        'color: #FFD700; font-size: 16px; font-weight: bold;')
        else:
            for i in range(5):
                if i in self.chapter_indicators:
    ...
    def _create_slot_widget(self, name, chapter_id):
        ...
        slot_frame.double_click_handler = lambda: self._on_slot_frame_double_clicked(
            slot_frame)
        self._update_slot_visual_state(slot_frame)
        return slot_frame
    ...
    def _update_slot_visual_state(self, slot_frame):
        ...
        slot_border_color = get_theme_color(
            self.local_config, 'border', 'white')
        direct_launch_slot_id = self.local_config.get(
            'direct_launch_slot_id', -1)
        is_direct_launch_slot = direct_launch_slot_id >= 0 and slot_frame.chapter_id >= 0 and (
        ...
        if getattr(slot_frame, 'is_selected', False):
            ...
            bg_color = slot_bg_color.replace(
                '0.75', '0.9').replace('150', '200')
        else:
            border_color = slot_border_color
    ...
    def _on_slot_clicked(self, slot_frame):
        ...
        if not is_chapter_mode:
            if slot_frame.assigned_mod:
                ...
                msg_box.setText(tr('ui.remove_mod_question',
                                mod_name=slot_frame.assigned_mod.name))
                msg_box.setStandardButtons(
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
                if msg_box.exec() == QMessageBox.StandardButton.Yes:
                    self._remove_mod_from_slot(
                        slot_frame, slot_frame.assigned_mod)
                    self._save_slots_state()
            else:
    ...
    def _on_slot_frame_double_clicked(self, slot_frame):
        ...
        if not is_chapter_mode or slot_frame.chapter_id < 0:
            return
        current_direct_launch_slot = self.local_config.get(
            'direct_launch_slot_id', -1)
        ...
        if is_direct_launch_active:
            msg_box.setText(tr('ui.disable_direct_launch',
                            chapter=slot_frame.chapter_id))
            msg_box.setStandardButtons(
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            ...
            msg_box.setText(tr('ui.enable_direct_launch',
                            chapter=slot_frame.chapter_id))
            msg_box.setStandardButtons(
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            msg_box.setDefaultButton(QMessageBox.StandardButton.No)
    ...
    def _update_installed_mods_for_chapter_mode(self, selected_chapter_id):
        ...
        is_demo_mode = hasattr(
            self, 'game_type_combo') and self.game_type_combo.currentData() == 'deltarunedemo'
        for mod_info in installed_mods:
            ...
            if mod_data:
                mod_widget = InstalledModWidget(
                    mod_data, is_local, is_available, parent=self)
                mod_widget.clicked.connect(self._on_installed_mod_clicked)
                mod_widget.remove_requested.connect(
                    self._on_installed_mod_remove)
                if selected_chapter_id is not None:
                    mod_widget.use_requested.connect(
                        lambda mod_data=mod_data: self._on_chapter_mode_mod_use(mod_data, selected_chapter_id))
                    is_in_slot = self._is_mod_in_specific_slot(
                        mod_data, selected_chapter_id)
                    mod_widget.set_in_slot(is_in_slot)
                else:
                    mod_widget.use_requested.connect(
                        self._on_installed_mod_use)
                self.installed_mods_layout.insertWidget(
                    self.installed_mods_layout.count() - 1, mod_widget)
        if self.installed_mods_layout.count() <= 1:
            if selected_chapter_id is not None:
                chapter_names = {-1: tr('ui.universal_slot'), 0: tr('ui.menu'), 1: tr(
                    'ui.chapter_1'), 2: tr('ui.chapter_2'), 3: tr('ui.chapter_3'), 4: tr('ui.chapter_4')}
                chapter_name = chapter_names.get(selected_chapter_id, tr(
                    'ui.chapter_n', chapter=str(selected_chapter_id)))
    ...
    def _mod_has_files_for_chapter(self, mod_data, chapter_id):
        try:
            mod_key = getattr(mod_data, 'key', None) or getattr(
                mod_data, 'mod_key', None)
            if not mod_key:
                return True
            ...
            if os.path.exists(config_path):
                try:
                    ...
                    pass
            chapter_folders = {-1: 'universal', 0: 'menu',
                               1: 'chapter1', 2: 'chapter2', 3: 'chapter3', 4: 'chapter4'}
    ...
    def _on_chapter_mode_mod_use(self, mod_data, chapter_id):
        ...
        for i in range(self.installed_mods_layout.count()):
            ...
            if item and item.widget():
                ...
                if hasattr(widget, 'mod_data') and hasattr(widget, 'use_button'):
                    ...
                    if widget_mod_data:
                        widget_mod_key = getattr(widget_mod_data, 'key', None) or getattr(
                            widget_mod_data, 'mod_key', None) or getattr(widget_mod_data, 'name', None)
                        current_mod_key = getattr(mod_data, 'key', None) or getattr(
                            mod_data, 'mod_key', None) or getattr(mod_data, 'name', None)
                        if widget_mod_key == current_mod_key:
                            mod_widget = widget
                            break
        status = getattr(mod_widget, 'status',
                         'ready') if mod_widget else 'ready'
        ...
        if target_slot and target_slot.assigned_mod:
            assigned_mod_key = getattr(target_slot.assigned_mod, 'key', None) or getattr(
                target_slot.assigned_mod, 'mod_key', None) or getattr(target_slot.assigned_mod, 'name', None)
            mod_key = getattr(mod_data, 'key', None) or getattr(
                mod_data, 'mod_key', None) or getattr(mod_data, 'name', None)
        ...
        if target_slot:
            ...
            QMessageBox.warning(self, tr('errors.error'),
                                tr('errors.target_slot_not_found'))
    ...
    def _show_mod_selection_for_slot(self, slot_frame):
        ...
        if not available_mods:
            QMessageBox.information(
                self, tr('ui.no_available_mods'), tr('ui.no_mods_to_insert'))
            return
        dialog = QDialog(self)
    ...
    def _update_installed_mods_display(self):
        if not hasattr(self, 'installed_mods_layout'):
            return
        is_chapter_mode = hasattr(
            self, 'chapter_mode_checkbox') and self.chapter_mode_checkbox.isChecked()
        if is_chapter_mode:
            if hasattr(self, 'selected_chapter_id') and self.selected_chapter_id is not None:
                self._update_installed_mods_for_chapter_mode(
    ...
    def _update_installed_mods_display_from_list(self, installed_mods):
            ...
            if selected_id is None:
                if hasattr(self, 'installed_mods_container') and hasattr(self, 'installed_mods_layout'):
                    ...
                    clear_layout_widgets(
                        self.installed_mods_layout, keep_last_n=1)
                    self._show_chapter_mode_instruction()
        ...
        for mod_info in installed_mods:
            ...
            if not is_local and is_available:
                public_mod = next(
                    (mod for mod in self.all_mods if mod.key == mod_info.get('key')), None)
                if public_mod:
                    has_update = any((self._mod_has_files_for_chapter(
                        public_mod, i) and self._get_mod_status_for_chapter(public_mod, i) == 'update' for i in range(5)))
            mod_data = self._create_mod_object_from_info(mod_info)
            if mod_data:
                mod_widget = InstalledModWidget(
                    mod_data, is_local, is_available, has_update, parent=self)
                mod_widget.clicked.connect(self._on_installed_mod_clicked)
                mod_widget.remove_requested.connect(
                    self._on_installed_mod_remove)
    ...
    def _refresh_installed_mods_async(self):
        ...
        try:
            ...
            self._installed_scan_thread.done.connect(
                self._update_installed_mods_display_from_list)
    ...
    def _show_empty_mods_message(self):
        show_empty_message_in_layout(self.installed_mods_layout, tr(
            'ui.empty'), self.local_config, font_size=18)

    def _show_empty_chapter_message(self, chapter_name):
        show_empty_message_in_layout(self.installed_mods_layout, tr(
            'ui.no_mods_for_chapter', chapter_name=chapter_name), self.local_config, font_size=16)
    ...
    def _cleanup_missing_mods(self, installed_mods):
        installed_mod_keys = {mod.get('mod_key')
                              for mod in installed_mods if mod.get('mod_key')}
        ...
        for orphaned_key in orphaned_keys:
            dummy_mod_data = self._create_mod_object_from_info(
                {'mod_key': orphaned_key, 'name': 'Orphaned Mod'})
            if not dummy_mod_data:
                continue
            self._remove_mod_from_all_slots(dummy_mod_data)
            config_keys = ['saved_slots_deltarune', 'saved_slots_deltarune_chapter',
                           'saved_slots_deltarunedemo', 'saved_slots_undertale']
            ...
            if os.path.exists(config_path):
                try:
                    ...
                    if config_data:
                        ...
                        if not mod_meta:
                            mods_metadata[mod_key] = {'installed_date': time.strftime(
                                '%Y-%m-%d %H:%M:%S'), 'is_available_on_server': not config_data.get('is_local_mod', False)}
                            metadata_updated = True
                            mod_meta = mods_metadata[mod_key]
                        config_data['installed_date'] = mod_meta.get(
                            'installed_date')
                        config_data['is_available_on_server'] = mod_meta.get(
                            'is_available_on_server', False)
                        config_data['is_local_mod'] = config_data.get(
                            'is_local_mod', False)
    ...
    def _on_installed_mod_remove(self, mod_data):
        try:
            ...
            msg_box.setText(
                tr('dialogs.delete_mod_confirmation', mod_name=mod_data.name))
            msg_box.setStandardButtons(
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            msg_box.setDefaultButton(QMessageBox.StandardButton.No)
            if msg_box.exec() == QMessageBox.StandardButton.Yes:
            ...
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.mod_removal_failed', error=str(e)))
    ...
    def _find_mod_in_slots(self, mod_data, exclude_chapter_id=None):
        ...
        for slot_frame in self.slots.values():
            if exclude_chapter_id is not None and slot_frame.chapter_id == exclude_chapter_id:
                continue
            if slot_frame.assigned_mod:
                assigned_mod_key = getattr(slot_frame.assigned_mod, 'key', None) or getattr(
                    slot_frame.assigned_mod, 'mod_key', None) or getattr(slot_frame.assigned_mod, 'name', None)
                if assigned_mod_key == mod_key:
                    return slot_frame
        return None
    ...
    def _show_slot_selection_dialog(self, mod_data):
        ...
        for key, slot_frame in self.slots.items():
            if slot_frame.assigned_mod is None:
                if slot_frame.chapter_id == -1:
                    ...
                    chapter_names = [tr('chapters.menu'), tr('tabs.chapter_1'), tr(
                        'tabs.chapter_2'), tr('tabs.chapter_3'), tr('tabs.chapter_4')]
        ...
        if not available_slots:
            QMessageBox.information(
                self, tr('dialogs.no_free_slots'), tr('dialogs.all_slots_occupied'))
            return
    ...
    def _show_mod_details_dialog(self, mod_data):
        ...
        dialog.setWindowTitle(
            tr('ui.mod_details_title', mod_name=mod_data.name))
        dialog.setMinimumSize(700, 700)
        dialog.resize(800, 750)
        secondary_text_color = get_theme_color(
            self.local_config, 'version_text', 'rgba(255, 255, 255, 178)')
        layout = QVBoxLayout(dialog)
        ...
        author_label = QLabel(
            f"""<span style="color: white;">{tr('ui.author_label')}</span> <span style="color: {secondary_text_color};">{author_text}</span>""")
        ...
        game_version_label = QLabel(
            f"""<span style="color: white;">{tr('ui.game_version_label')}</span> <span style="color: {secondary_text_color};">{game_version_text}</span>""")
        ...
        created_label = QLabel(
            f"""<span style="color: white;">{tr('ui.created_label')}</span> <span style="color: {secondary_text_color};">{created_date_text}</span>""")
        ...
        updated_label = QLabel(
            f"""<span style="color: white;">{tr('ui.updated_label')}</span> <span style="color: {secondary_text_color};">{updated_date_text}</span>""")
        updated_label.setStyleSheet('font-size: 12px;')
        metadata_layout.addWidget(updated_label)
        downloads_label = QLabel(
            f"""<span style="color: white;">{tr('ui.downloads_label')}</span> <span style="color: {secondary_text_color};">{mod_data.downloads}</span>""")
        ...
        if hasattr(mod_data, 'tags') and mod_data.tags:
            ...
            tag_translations = {'translation': tr('tags.translation'), 'customization': tr(
                'tags.customization'), 'gameplay': tr('tags.gameplay'), 'other': tr('tags.other')}
            tags_list = mod_data.tags if isinstance(
                mod_data.tags, list) else [mod_data.tags]
            filtered_tags = [tag for tag in tags_list if tag]
            translated_tags = [tag_translations.get(
                tag, tag) or tag for tag in filtered_tags]
            for tag in translated_tags:
                tag_label = QLabel(tag)
                tag_label.setStyleSheet(
                    f'font-size: 12px; color: {secondary_text_color}; margin-left: 10px;')
        ...
        if hasattr(mod_data, 'gamebanana_url') and mod_data.gamebanana_url:
            ...
            gb_button.clicked.connect(
                lambda: webbrowser.open(mod_data.gamebanana_url))
        ...
        mod_version = mod_data.version.split(
            '|')[0] if mod_data.version and '|' in mod_data.version else mod_data.version
        version_text = mod_version or 'N/A'
        version_label = QLabel(
            tr('ui.mod_version_label', version_text=version_text))
        version_label.setStyleSheet(
            f'font-size: 14px; color: {secondary_text_color}; margin-bottom: 10px;')
        ...
        if getattr(mod_data, 'is_verified', False):
            ...
            verified_desc.setStyleSheet(
                'color: #4CAF50; font-size: 11px; margin-left: 12px;')
        ...
        if getattr(mod_data, 'is_xdelta', False):
            ...
            patching_desc.setStyleSheet(
                'color: #2196F3; font-size: 11px; margin-left: 12px;')
            ...
            replacement_desc.setStyleSheet(
                'color: #FF9800; font-size: 11px; margin-left: 12px;')
        ...
        if hasattr(mod_data, 'description_url') and mod_data.description_url:
            self._load_description_from_url(
                desc_text, mod_data.description_url)
        else:
    ...
    def _load_description_from_url(self, text_widget, description_url):
        try:
            ...
            if response.ok:
                ...
                is_markdown = description_url.lower().endswith(('.md', '.markdown')
                                                               ) or '# ' in content or '## ' in content or ('**' in content) or ('__' in content)
                if is_markdown:
                ...
                text_widget.setPlainText(
                    tr('errors.description_http_error_code', code=response.status_code))
        except Exception as e:
            text_widget.setPlainText(
                tr('errors.description_load_error_details', error=str(e)))
    ...
    def _assign_mod_to_slot(self, slot_frame, mod_data, save_state=True):
        ...
        if is_large_slot:
            ...
            name_label.setStyleSheet(
                'font-weight: bold; font-size: 13px; border: none; background: transparent;')
            name_label.setText(mod_data.name)
            if is_local_mod:
                status_text, status_color = (tr('status.local_mod'), '#FFD700')
            else:
                needs_update = any((self._mod_has_files_for_chapter(
                    mod_data, i) and self._get_mod_status_for_chapter(mod_data, i) == 'update' for i in range(5)))
                status_text, status_color = (tr('status.update_available'), 'orange') if needs_update else (
                    tr('status.version_current'), 'lightgreen')
            ...
            display_name = original_name[:7] + \
                '...' if len(original_name) > 10 else original_name
            ...
            if is_local_mod:
                ...
                needs_update = any((self._mod_has_files_for_chapter(
                    mod_data, i) and self._get_mod_status_for_chapter(mod_data, i) == 'update' for i in range(5)))
                status_text, status_color = (tr('status.update_short'), 'orange') if needs_update else (
                    tr('status.current_short'), 'lightgreen')
            version_label = QLabel(status_text)
            version_label.setStyleSheet(
                f'color: {status_color}; font-size: 9px; border: none; background: transparent;')
    ...
    def _update_mod_widgets_slot_status(self):
        ...
        for i in range(self.installed_mods_layout.count() - 1):
            ...
            if item:
                widget = item.widget()
                if isinstance(widget, InstalledModWidget):
                    is_in_slot = self._find_mod_in_slots(
                        widget.mod_data) is not None
            ...
            if slot_frame.assigned_mod and slot_frame.content_widget:
                ...
                if hasattr(slot_frame, 'mod_icon') and slot_frame.mod_icon:
                    load_mod_icon_universal(
                        slot_frame.mod_icon, slot_frame.assigned_mod, 32)
            ...
            for folder_name in os.listdir(self.mods_dir):
                ...
                if os.path.exists(config_path):
                    try:
                        ...
                        logging.warning(
                            f'Failed to read installed mod config {config_path}: {e}')
                        continue
    ...
    def _sort_filtered_mods(self):
        ...
        if sort_type == 0:
            self.filtered_mods.sort(key=lambda mod: getattr(
                mod, 'downloads', 0), reverse=reverse)
        elif sort_type == 1:
            self.filtered_mods.sort(key=lambda mod: self._parse_date(
                getattr(mod, 'last_updated', '')), reverse=reverse)
        elif sort_type == 2:
            self.filtered_mods.sort(key=lambda mod: self._parse_date(
                getattr(mod, 'created_date', '')), reverse=reverse)
    ...
    def _update_mod_display(self):
        ...
        try:
            for mod in current_page_mods:
                ...
                plaque.install_requested.connect(
                    self._on_mod_install_requested)
                plaque.uninstall_requested.connect(
                    self._on_mod_uninstall_requested)
                plaque.clicked.connect(self._on_mod_clicked)
                plaque.details_requested.connect(
                    self._on_mod_details_requested)
                plaque.install_button.setEnabled(not self.is_installing)
                self.mod_list_layout.insertWidget(
                    self.mod_list_layout.count() - 1, plaque)
        finally:
    ...
    def _install_single_mod(self, mod, force=False):
        try:
            ...
            if not available_chapters:
                ...
                QMessageBox.warning(self, tr('errors.error'), tr(
                    'errors.mod_no_files', mod_name=mod.name) + f'\n\nDebug: {debug_info}')
                return
            ...
            if not is_xdelta_mod and (not was_installed_before):
                reply = QMessageBox.question(self, tr('dialogs.file_replacement_warning_title'), tr(
                    'dialogs.file_replacement_warning_body'), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
                if reply != QMessageBox.StandardButton.Yes:
                    self.update_status_signal.emit(
                        tr('status.install_cancelled_by_user'), UI_COLORS['status_info'])
                    return
            install_tasks = [(mod, chapter_id)
            ...
            self.current_install_thread = InstallModsThread(
                self, install_tasks, was_installed_before)
            self.install_thread = self.current_install_thread
            self.install_thread.progress.connect(
                lambda v, oid=op_id: self._on_install_progress_token(v, oid))
            self.install_thread.status.connect(
                lambda msg, col, oid=op_id: self._on_install_status_token(msg, col, oid))
            self.install_thread.finished.connect(
                lambda ok, oid=op_id: self._on_install_finished_token(ok, oid))
            self.progress_bar.setVisible(True)
            self.progress_bar.setValue(0)
            try:
                self.update_status_signal.emit(
            ...
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.mod_install_failed', error=str(e)))
    ...
    def _on_single_mod_install_finished(self, success):
        ...
        if hasattr(self, 'current_install_thread') and self.current_install_thread:
            was_installed_before = getattr(
                self.current_install_thread, 'was_installed_before', False)
        self.progress_bar.setValue(0)
        self.progress_bar.setVisible(False)
        if success:
            self.update_status_signal.emit(
        ...
        if success:
            ...
            if hasattr(self, '_update_installed_mods_display'):
                self._update_installed_mods_display()
            QTimer.singleShot(
                100, self._refresh_specific_mod_widget_after_update)
            if not was_installed_before:
                QMessageBox.information(self, tr('dialogs.mod_installed_title'), tr(
                    'dialogs.mod_installed_apply_info'))
            self.update_status_signal.emit(
                tr('status.mod_installed_success'), UI_COLORS['status_success'])
        self._update_action_button_state()
    ...
    def _refresh_specific_mod_widget_after_update(self):
        if not hasattr(self, 'current_install_thread') or not self.current_install_thread:
            return
        install_tasks = getattr(
            self.current_install_thread, 'install_tasks', [])
    ...
    def _on_mod_uninstall_requested(self, mod):
        ...
        reply = QMessageBox.question(self, tr('dialogs.delete_confirmation'), tr('dialogs.delete_mod_confirmation',
                                     mod_name=mod.name), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
    ...
    def _uninstall_single_mod(self, mod):
        try:
            ...
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.mod_delete_failed', error=str(e)))
    ...
    def _on_mod_install_finished(self, success, from_gb=False):
        ...
        if success:
            ...
            if hasattr(self, 'pending_updates') and self.pending_updates:
                ...
                self.update_status_signal.emit(
                    tr('status.updating_mod', mod_name=next_mod.name), UI_COLORS['status_warning'])
    ...
    def _prompt_for_mods_dir(self):
        ...
        new_parent_dir = QFileDialog.getExistingDirectory(
            self, tr('ui.select_new_mods_folder'), os.path.dirname(current_mods_dir))
        ...
        if os.path.exists(new_mods_dir):
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.mods_folder_exists', dir=new_parent_dir))
            return
        try:
            self.update_status_signal.emit(
                tr('status.moving_mods_folder'), UI_COLORS['status_warning'])
            QApplication.processEvents()
            shutil.move(current_mods_dir, new_mods_dir)
            self.mods_dir = new_mods_dir
            self.local_config['mods_dir_path'] = new_parent_dir
            self._write_local_config()
            QMessageBox.information(self, tr('dialogs.success'), tr(
                'dialogs.mods_folder_moved', path=new_mods_dir))
            self.update_status_signal.emit(
                tr('status.mods_folder_location_changed'), UI_COLORS['status_success'])
        except Exception as e:
            QMessageBox.critical(self, tr('dialogs.move_error'), tr(
                'dialogs.mods_folder_move_failed', error=str(e)))
            self.mods_dir = current_mods_dir
            self.update_status_signal.emit(
                tr('status.mods_folder_change_error'), UI_COLORS['status_error'])
    ...
    def _on_toggle_full_install(self, state):
        ...
        if platform.system() == 'Darwin' and self.is_full_install:
            QMessageBox.information(self, tr('dialogs.unavailable'), tr(
                'dialogs.macos_install_unavailable'))
            self.full_install_checkbox.blockSignals(True)
    ...
    def apply_theme(self):
        ...
        background_disabled = self.local_config.get(
        ...
        if not background_disabled:
            background_path = self.local_config.get('custom_background_path') or resource_path(
                f"resources/{theme.get('background', '')}")
            if background_path:
        ...
        button_color = self.local_config.get(
            'custom_color_button') or theme['colors']['button']
        border_color = self.local_config.get(
            'custom_color_border') or theme['colors']['border']
        button_hover_color = self.local_config.get(
            'custom_color_button_hover') or theme['colors']['button_hover']
        main_text_color = self.local_config.get(
            'custom_color_text') or theme['colors']['text']
        ...
        explicit_color_widgets = [
            getattr(self, 'telegram_button', None), getattr(self, 'discord_button', None)]
        ...
        scroll_handle_color = self.local_config.get(
            'custom_color_button') or 'white'
        ...
        (app_inst if isinstance(app_inst, QApplication)
         else self).setStyleSheet(style_sheet)
    ...
    def _configure_hidden_tab_bar(self, tab_widget: QTabWidget):
        ...
        if bar:
            ...
            bar.setSizePolicy(QSizePolicy.Policy.Fixed,
    ...
    def _init_save_manager_ui(self):
        ...
        for ch in range(1, 5):
            ...
            for s in range(3):
                ...
                lbl.setStyleSheet(
                    'border:1px solid white; background-color: rgba(0,0,0,128); padding:4px;')
            ...
            v.addStretch()
            self.save_tabs.addTab(
                tab, tr('ui.chapter_tab_title', chapter_num=ch))
        ...
        self.left_col_btn.clicked.connect(
            lambda: self._navigate_collection(-1))
        bottom.addWidget(self.left_col_btn)
        self.switch_collection_btn = QPushButton(
            tr('buttons.additional_slots'))
        self.switch_collection_btn.clicked.connect(
            self._toggle_collection_view)
        bottom.addWidget(self.switch_collection_btn)
        self.right_col_btn = QPushButton('→')
        self.right_col_btn.clicked.connect(
            lambda: self._navigate_collection(1))
        bottom.addWidget(self.right_col_btn)
        lay.addLayout(bottom)
        self.rename_collection_btn = QPushButton(
            tr('buttons.rename_collection'))
        self.rename_collection_btn.clicked.connect(
            self._rename_current_collection)
        self.delete_collection_btn = QPushButton(
            tr('buttons.delete_collection'))
        self.delete_collection_btn.clicked.connect(
            self._delete_current_collection)
        ...
        self.copy_from_main_btn.clicked.connect(
            lambda: self._copy_between_storages(to_collection=True))
        copy_bar.addWidget(self.copy_from_main_btn)
        self.copy_to_main_btn = QPushButton(tr('buttons.copy_to_main'))
        self.copy_to_main_btn.clicked.connect(
            lambda: self._copy_between_storages(to_collection=False))
        ...
        self.import_btn.clicked.connect(
            lambda: self._action_import_export(True))
        self.export_btn.clicked.connect(
            lambda: self._action_import_export(False))
        lay.addLayout(self.slot_actions)
        top.addWidget(self.rename_collection_btn)
        top.addWidget(self.delete_collection_btn)
        self.save_tabs.currentChanged.connect(
            lambda _: self._on_chapter_tab_changed())
    ...
    def eventFilter(self, obj, ev):
        if obj is self.save_manager_widget and ev.type() == QEvent.Type.MouseButtonPress:
            click_pos = ev.pos()
            inside = any((lbl.rect().contains(lbl.mapFrom(
                self.save_manager_widget, click_pos)) for lbl in self._slot_labels.values()))
            if not inside:
                self._clear_selected_slot()
        return super().eventFilter(obj, ev)

    def _update_slot_action_bar(self):
        in_main = self.current_collection_idx.get(
            self.save_tabs.currentIndex() + 1, -1) == -1
    ...
    def _update_slot_highlight(self):
        ...
        for (ch, sl), lbl in self._slot_labels.items():
            if self.selected_slot == (ch, sl):
                lbl.setStyleSheet(
                    f'border:2px solid white; background-color: {slot_bg}; padding:4px;')
            else:
                lbl.setStyleSheet(
                    f'border:1px solid white; background-color: {slot_bg}; padding:4px;')
    ...
    def _return_from_save_manager(self):
        ...
        try:
            self.settings_button.clicked.disconnect(
                self._return_from_save_manager)
        except TypeError:
            pass
        self.settings_button.clicked.connect(self._toggle_settings_view)
    ...
    def _on_configure_saves_click(self):
        ...
        self._refresh_save_slots()
        self.update_status_signal.emit(
            tr('status.save_path_info', save_path=self.save_path), UI_COLORS['status_info'])
        ...
        for s in range(3):
            ...
            if active:
                ...
                finished = os.path.exists(
                    fin_fp) and os.path.getsize(fin_fp) > 0
                status = tr('status.completed_save') if finished else tr(
                    'status.incomplete_save')
                text = tr('ui.save_info', nickname=nickname,
    ...
    def _prompt_for_save_path(self) -> bool:
        ...
        if not is_valid_save_path(path):
            QMessageBox.warning(self, tr('errors.empty_folder_title'), tr(
                'errors.empty_folder_message'))
            return False
        self.save_path = path
        self.local_config['save_path'] = self.save_path
        self._write_local_config()
    ...
    def _create_new_collection(self, chapter: int) -> bool:
        ...
        try:
            ...
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.folder_creation_failed', error=str(e)))
            return False
    ...
    def _update_collection_ui(self):
        ...
        in_col = idx != -1
        cols = self._list_collections(chapter)
        self.switch_collection_btn.setText(
            tr('buttons.main_slots') if in_col else tr('buttons.additional_slots'))
    ...
    def _rename_current_collection(self):
        ...
        new_name, ok = QInputDialog.getText(self, tr(
            'dialogs.change_collection_name'), tr('dialogs.new_name'), text=old_name)
        ...
        try:
            ...
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.rename_failed', error=str(e)))
    ...
    def _delete_current_collection(self):
        ...
        try:
            ...
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.deletion_failed', error=str(e)))
    ...
    def _copy_between_storages(self, to_collection: bool):
        ...
        if idx == -1:
            return
        src_dir = self.save_path if to_collection else self._get_collection_path(
            chapter, idx)
        dst_dir = self._get_collection_path(
            chapter, idx) if to_collection else self.save_path
        if not src_dir or not dst_dir:
            return
        prompt = (tr('dialogs.overwrite_all_3_slots_collection') if to_collection else tr('dialogs.overwrite_all_3_main_slots')) if self.selected_slot is None else tr(
            'dialogs.overwrite_selected_slot_collection') if to_collection else tr('dialogs.overwrite_selected_main_slot')
        reply = QMessageBox.question(self, tr('dialogs.copy_confirmation'),
                                     prompt, QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply != QMessageBox.StandardButton.Yes:
            return
        try:
            for slot_idx in slot_indices:
                finish_idx = SAVE_SLOT_FINISH_MAP.get(slot_idx, -1)
                names = [f'filech{chapter}_{slot_idx}',
                         f'filech{chapter}_{finish_idx}']
                for _name in names:
            ...
            self._refresh_save_slots()
            self.update_status_signal.emit(
                tr('status.copying_completed'), UI_COLORS['status_success'])
        except Exception as e:
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.copy_failed', error=str(e)))
            self.update_status_signal.emit(
                tr('status.copying_error'), UI_COLORS['status_error'])
    ...
    def _action_import_export(self, is_import: bool):
        ...
        choice, ok = QInputDialog.getItem(self, tr('dialogs.where_to') if not is_import else tr('dialogs.where_from'), tr('ui.select_storage'), [tr(
            'dialogs.external_file') if is_import else tr('dialogs.external_folder'), tr('dialogs.additional_collection') if idx == -1 else tr('dialogs.main_slots')], 0, False)
        if not ok:
            return
        if choice in [tr('dialogs.external_file'), tr('dialogs.external_folder')]:
            if is_import:
                ...
                if not re.fullmatch(f'filech{ch}_[0-2]', os.path.basename(fp)):
                    QMessageBox.warning(
                        self, tr('errors.invalid_file'), tr('errors.wrong_save_file'))
                    return
                ...
                dir_ = QFileDialog.getExistingDirectory(
                    self, tr('dialogs.export_save_location'))
                if not dir_:
                    return
                if not os.path.exists(src_fp):
                    QMessageBox.warning(
                        self, tr('errors.no_save'), tr('errors.empty_slot'))
                    return
            ...
            if idx == -1:
                ...
                sel, ok = QInputDialog.getItem(
                    self, tr('ui.collections'), tr('ui.select'), cols, 0, False)
                if not ok:
                    return
            ...
            if is_import:
                if not os.path.exists(target_main_fp):
                    QMessageBox.warning(
                        self, tr('errors.no_save'), tr('errors.no_import_save'))
                    return
    ...
    def _on_bg_ready(self, obj):
        if isinstance(obj, tuple):
            if obj[0] == 'gif':
                ...
                self.background_movie = None
                self.background_pixmap = QPixmap.fromImage(obj[1]).scaled(self.size(
    ...
    def paintEvent(self, event):
        ...
        if self.background_movie is not None:
            painter.drawPixmap(
                self.rect(), self.background_movie.currentPixmap())
        elif self.background_pixmap:
            painter.drawPixmap(self.rect(), self.background_pixmap)
        else:
            bg_color_str = self.local_config.get(
                'custom_color_background') or 'rgba(0, 0, 0, 200)'
            try:
    ...
    def _on_reset_settings_click(self):
        reply = QMessageBox.question(self, tr('dialogs.reset_settings_confirm_title'), tr(
            'dialogs.reset_settings_confirm_text'), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            self._stop_background_music()
            language = self.local_config.get('language', 'en')
            custom_files = [os.path.join(self.config_dir, 'custom_background_music.mp3'), os.path.join(self.config_dir, 'custom_background_music.wav'), os.path.join(
                self.config_dir, 'custom_startup_sound.mp3'), os.path.join(self.config_dir, 'custom_startup_sound.wav')]
            for file_path in custom_files:
                if os.path.exists(file_path):
                    os.remove(file_path)
            self.local_config.clear()
            self.local_config['language'] = language
            config_keys_to_clear = ['saved_slots_deltarune', 'saved_slots_deltarune_chapter',
                                    'saved_slots_deltarunedemo', 'saved_slots_undertale']
            ...
            self._update_action_button_state()
            self.background_music_button.setText(
                self._get_background_music_button_text())
            self.startup_sound_button.setText(
                self._get_startup_sound_button_text())
            QMessageBox.information(self, tr('dialogs.success'), tr(
                'status.settings_reset_success'))
            self.update()
            self.repaint()
    ...
    def _on_background_button_click(self):
        if self.local_config.get('custom_background_path'):
            ...
            filepath, _ = QFileDialog.getOpenFileName(self, tr(
                'ui.select_background_image'), '', get_file_filter('background_images'))
            if not filepath:
                return
    ...
    def _update_background_button_state(self):
        ...
        self.change_background_button.setText(tr('buttons.remove_background') if self.local_config.get(
            'custom_background_path') else tr('buttons.change_background'))
    ...
    def _load_help_content(self):
        if localization_manager.get_current_language() == 'ru':
            help_url = self.global_settings.get(
                'help_ru_url', self.global_settings.get('help_url', ''))
        else:
            help_url = self.global_settings.get(
                'help_en_url', self.global_settings.get('help_url', ''))
        if not help_url:
            self.help_text_edit.setMarkdown(
                f"<i>{tr('dialogs.help_not_available')}</i>")
            return
        self.help_text_edit.setMarkdown(f"<i>{tr('status.loading')}</i>")
    ...
    def _update_settings_page_visibility(self):
        ...
        self.settings_pages_container.setVisible(
            not is_changelog and (not is_help))
    ...
    def _on_custom_style_edited(self):
        for key, widget in self.color_widgets.items():
            ...
            self.local_config[config_key] = color if self._is_valid_hex_color(
                color) else ''
        self._write_local_config()
        self.apply_theme()
            ...
            if search_tab:
                ...
                if layout and layout.count() > 0:
                    ...
                    if filters and filters.objectName() == 'filters':
                        filter_bg_color = self.local_config.get(
                            'custom_color_background') or 'rgba(0, 0, 0, 150)'
                        filter_border_color = self.local_config.get(
                            'custom_color_border') or 'white'
                        filters.setStyleSheet(
                            f'QFrame#filters {{ background-color: {filter_bg_color}; border: 2px solid {filter_border_color}; padding: 8px; }}')
                elif layout:
            ...
            if os.path.exists(splash_path):
                ...
                if not pixmap.isNull():
                    ...
                    scaled_pixmap = pixmap.scaled(
                        target_w, target_h, Qt.AspectRatioMode.KeepAspectRatioByExpanding, Qt.TransformationMode.SmoothTransformation)
    ...
    def _on_background_music_button_click(self):
        ...
        if custom_exists:
            ...
            file_path, _ = QFileDialog.getOpenFileName(
                self, tr('dialogs.select_background_music'), '', 'Audio Files (*.mp3 *.wav)')
            if file_path:
                lower = file_path.lower()
                if not (lower.endswith('.mp3') or lower.endswith('.wav')):
                    QMessageBox.warning(
                        self, tr('errors.error'), 'Можно выбрать только MP3 или WAV файл')
                    return
                try:
                    self._stop_background_music()
                    os.makedirs(self.config_dir, exist_ok=True)
                    ...
                    dest_path = os.path.join(
                        self.config_dir, f'custom_background_music{ext}')
                    shutil.copy2(file_path, dest_path)
                    self.background_music_button.setText(
                        self._get_background_music_button_text())
                    self._maybe_start_background_music()
                    QMessageBox.information(self, tr('dialogs.success'), tr(
                        'dialogs.background_music_selected'))
                except Exception as e:
    ...
    def _on_startup_sound_button_click(self):
        ...
        if existing:
            ...
            file_path, _ = QFileDialog.getOpenFileName(
                self, tr('dialogs.select_startup_sound'), '', 'Audio Files (*.mp3 *.wav)')
            if file_path:
                lower = file_path.lower()
                if not (lower.endswith('.mp3') or lower.endswith('.wav')):
                    QMessageBox.warning(
                ...
                try:
                    ...
                    ext = '.mp3' if lower.endswith('.mp3') else '.wav'
                    dest = os.path.join(
                        self.config_dir, f'custom_startup_sound{ext}')
                    shutil.copy2(file_path, dest)
                    self.startup_sound_button.setText(
                        self._get_startup_sound_button_text())
                    QMessageBox.information(self, tr('dialogs.success'), tr(
                        'dialogs.startup_sound_selected'))
                except Exception as e:
    ...
    def _on_toggle_direct_launch_for_slot(self, slot_id):
        ...
        if self.local_config.get('launch_via_steam', False):
            QMessageBox.warning(self, tr('dialogs.incompatibility'), tr(
                'dialogs.direct_launch_steam_incompatible'))
            return
        if platform.system() == 'Darwin':
            QMessageBox.warning(self, tr('dialogs.incompatibility'), tr(
                'dialogs.direct_launch_macos_incompatible'))
            return
    ...
    def _update_action_button_state(self):
        ...
        if not self.initialization_completed:
            ...
            return
        is_demo_mode = isinstance(self.game_mode, DemoGameMode)
        is_full_install_enabled = is_demo_mode and hasattr(
            self, 'full_install_checkbox') and self.full_install_checkbox.isChecked()
    ...
    def _initialize_mutual_exclusions(self):
        is_direct_launch = self.local_config.get(
            'direct_launch_slot_id', -1) >= 0 and self.game_mode.direct_launch_allowed and (platform.system() != 'Darwin')
    ...
    def _post_show_initialization(self):
        ...
        if localization_manager.get_current_language() == 'ru':
            changelog_url = self.global_settings.get(
                'changelog_ru_url', self.global_settings.get('changelog_url'))
        else:
            changelog_url = self.global_settings.get(
                'changelog_en_url', self.global_settings.get('changelog_url'))
        if changelog_url:
            changelog_thread = FetchChangelogThread(
                changelog_url.strip(), self)
            changelog_thread.finished.connect(
                self.changelog_text_edit.setMarkdown)
            changelog_thread.start()
        else:
            self.changelog_text_edit.setMarkdown(
                tr('status.changelog_load_failed'))
        ...
        self.disable_background_checkbox.setChecked(
            self.local_config.get('background_disabled', False))
        self.disable_background_checkbox.blockSignals(False)
        self.disable_splash_checkbox.blockSignals(True)
        self.disable_splash_checkbox.setChecked(
        ...
        self._update_background_button_state()
        self._migrate_config_if_needed()
        self.use_custom_executable_checkbox.setChecked(
            self.local_config.get('use_custom_executable', False))
        self.launch_via_steam_checkbox.setChecked(
            self.local_config.get('launch_via_steam', False))
    ...
    def _check_and_manage_steam_deck_saves(self):
        ...
        try:
            ...
            proton_save_path = os.path.join(home_dir, '.steam', 'steam', 'steamapps', 'compatdata',
                                            steam_app_id, 'pfx', 'drive_c', 'users', 'steamuser', 'AppData', 'Local', game_name)
            ...
            if os.path.lexists(native_save_path):
                ...
                if os.path.isdir(native_save_path) and (not os.listdir(native_save_path)):
                    ...
                    QMessageBox.information(self, tr('dialogs.backup'), tr(
                        'dialogs.backup_created_for_steam_deck', backup_path=backup_path))
            os.symlink(proton_save_path, native_save_path)
            QMessageBox.information(self, tr('dialogs.steam_deck_setup'), tr(
                'dialogs.steam_deck_compatibility_configured'))
        except Exception as e:
    ...
    def _check_for_launcher_updates(self):
        ...
        try:
            ...
            if not remote_version or _vkey(remote_version) <= _vkey(LAUNCHER_VERSION):
                ...
                return
            platform_key_map = {'Windows': 'windows',
                                'Linux': 'linux', 'Darwin': f'macos-{ARCH}'}
            current_platform_key = platform_key_map.get(platform.system())
            download_url = launcher_files.get(
                'urls', {}).get(current_platform_key)
            ...
            if not download_url:
                self.update_status_signal.emit(
                    tr('errors.no_build_for_os', platform=current_platform_key), UI_COLORS['status_warning'])
                return
            update_info = {'version': remote_version, 'url': download_url, 'message': update_message,
                           'message_ru': update_message_ru, 'message_en': update_message_en}
            self.update_info_ready.emit(update_info)
        except requests.RequestException as e:
            self.update_status_signal.emit(
                tr('errors.update_check_network_error', error=str(e)), UI_COLORS['status_error'])
        except Exception as e:
            self.update_status_signal.emit(
                tr('errors.update_check_general_error', error=str(e)), UI_COLORS['status_error'])
    ...
    def _handle_update_info(self, update_info):
        if self.initialization_completed and getattr(self, 'is_shown_to_user', False):
            ...
            QTimer.singleShot(
                1000, lambda: self._handle_update_info(update_info))
        ...
        try:
            ...
            if legacy_path and os.path.isdir(legacy_path):
                try:
                    ...
                    pass
                QMessageBox.information(self, tr('dialogs.legacy_cleanup_title'), tr(
                    'dialogs.legacy_cleanup_message'))
        except Exception:
            pass
    ...
    def _prompt_for_update(self, update_info):
        ...
        update_message = tr('dialogs.new_version_banner', version=update_info['version']) + '<br>' + tr(
            'dialogs.current_version_banner', current_version=LAUNCHER_VERSION)
        if localization_manager.get_current_language() == 'ru':
            message_text = update_info.get(
                'message_ru') or update_info.get('message', '')
        else:
            message_text = update_info.get(
                'message_en') or update_info.get('message', '')
        ...
        reply = QMessageBox.question(
            self, tr('status.update_available'), update_message)
        if reply == QMessageBox.StandardButton.Yes:
    ...
    def _perform_update(self, update_info):
        ...
        self.progress_bar.setVisible(True)
        self.progress_bar.setValue(0)
        threading.Thread(target=self._update_worker, args=(
            update_info,), daemon=True).start()
        ...
        try:
            with tempfile.TemporaryDirectory(prefix='deltahub-update-') as tmp_dir:
                archive_path = os.path.join(
                    tmp_dir, 'update' + os.path.splitext(update_info['url'].split('?')[0])[1])
                self.update_status_signal.emit(
                    tr('status.downloading_version', version=update_info['version']), UI_COLORS['status_warning'])
                response = requests.get(
                ...
                if system == 'Windows':
                    new_exe_path = next((os.path.join(root, f) for root, _, files in os.walk(
                        extraction_dir) for f in files if f.lower().endswith('.exe')), None)
                    if not new_exe_path:
                        raise RuntimeError(
                            tr('errors.exe_not_found_in_archive'))
                ...
                replace_target = os.path.abspath(os.path.join(os.path.dirname(
                    current_exe_path), '..', '..')) if system == 'Darwin' else current_exe_path
                backup_path = f'{replace_target}.old'
                if system == 'Darwin':
                    if archive_path.lower().endswith('.zip'):
                        subprocess.run(
                            ['/usr/bin/ditto', '-x', '-k', archive_path, extraction_dir], check=True)
                    new_content_path = next((os.path.join(extraction_dir, d) for d in os.listdir(
                        extraction_dir) if d.endswith('.app')), None)
                    if new_content_path is None:
                        raise RuntimeError(
                            tr('errors.app_not_found_after_unpack'))
                    fix_macos_python_symlink(Path(new_content_path))
                else:
                    new_content_path = next((os.path.join(root, file) for root, _, files in os.walk(extraction_dir) for file in files if os.path.isfile(
                        os.path.join(root, file)) and os.access(os.path.join(root, file), os.X_OK)), None)
                    if new_content_path is None or not os.path.exists(new_content_path):
                        raise RuntimeError(
                            tr('errors.executable_not_found_after_unpack'))
            ...
            self.error_signal.emit(
                tr('dialogs.update_permission_error_details'))
        except Exception as e:
            self.update_status_signal.emit(
                tr('errors.update_failed', error=str(e)), UI_COLORS['status_error'])
            self.error_signal.emit(
                tr('errors.update_could_not_complete', error=str(e)))
        finally:
    ...
    def _on_refresh_clicked(self, is_initial=False):
        ...
        threading.Thread(
            target=self._check_for_launcher_updates, daemon=True).start()
    ...
    def _on_fetch_translations_finished(self, success: bool):
        try:
            ...
            if success:
                ...
                fallback_msg = tr('ui.network_fallback_message') if self.all_mods else tr(
                    'ui.network_update_failed')
                self.update_status_signal.emit(
                    fallback_msg, UI_COLORS['status_error'])
        except Exception as e:
            self.update_status_signal.emit(
                tr('errors.mod_list_processing_error', error=str(e)), UI_COLORS['status_error'])
    ...
    def _refresh_mods_in_slots(self):
        ...
        for slot_frame in self.slots.values():
            if slot_frame.assigned_mod:
                ...
                mod_key = getattr(old_mod, 'key', None) or getattr(
                    old_mod, 'mod_key', None)
                for updated_mod in self.all_mods:
                    updated_mod_key = getattr(updated_mod, 'key', None) or getattr(
                        updated_mod, 'mod_key', None)
    ...
    def _perform_full_install(self):
        ...
        base_dir = QFileDialog.getExistingDirectory(
            self, tr('dialogs.install_demo_location'))
        if not base_dir:
            self.action_button.setEnabled(True)
            return
        ...
        self.full_install_thread.progress.connect(self.progress_bar.setValue)
        self.full_install_thread.finished.connect(
            self._on_full_install_finished)
    ...
    def _run_as_admin_windows(self, path: str) -> bool:
        ...
        try:
            ...
            self.update_status_signal.emit(
                tr('status.permission_change_failed'), UI_COLORS['status_error'])
            return False
    ...
    def _prepare_game_files(self, selections: Dict[int, str]) -> bool:
        try:
            ...
            for ui_index, mod_key in selections.items():
                ...
                mod = next(
                    (m for m in self.all_mods if m.key == mod_key), None)
                if not mod:
                    continue
                ...
                if not os.path.isdir(source_dir):
                    self.update_status_signal.emit(
                        tr('errors.mod_folder_not_found', mod_name=mod.name, path=source_dir), UI_COLORS['status_warning'])
                    continue
                mod_type_str = tr('ui.mod_type_local') if is_local else tr(
                    'ui.mod_type_public')
                self.update_status_signal.emit(
                    tr('status.applying_mod', mod_name=mod.name, mod_type=mod_type_str), UI_COLORS['status_warning'])
                print(
                    f'[XDELTA-DEBUG] UI index={ui_index}, chapter_id={chapter_id}, mod_key={mod_key}, mod_name={mod.name}')
                print(f'[XDELTA-DEBUG] source_dir={source_dir}')
                if chapter_id in applied_chapters:
                    continue
                is_xdelta_mod = self._is_xdelta_mod(
                    mod, source_dir, chapter_id)
                ...
                if not target_dir:
                    print(
                        f'[XDELTA-DEBUG] target_dir not found for chapter_id={chapter_id}')
                    continue
                print(f'[XDELTA-DEBUG] target_dir={target_dir}')
                if not ensure_writable(target_dir):
                    raise PermissionError(
                        tr('errors.no_write_permission_for', path=target_dir))
            ...
            path = e.filename or (
                e.args[0] if e.args else tr('errors.unknown_path'))
    ...
    def _is_xdelta_mod(self, mod_info, source_dir: str, chapter_id: Optional[int] = None) -> bool:
        ...
        if chapter_id is not None:
            ...
            search_dir = source_dir
        print(
            f'[XDELTA-DEBUG] _is_xdelta_mod: chapter_id={chapter_id}, search_dir={search_dir}')
    ...
    def _create_backup_and_copy_mod_files(self, source_dir: str, target_dir: str, chapter_id: Optional[int] = None, mod_info=None):
        if not os.path.isdir(source_dir):
            self.update_status_signal.emit(
                tr('errors.mod_folder_not_found_simple', path=source_dir), UI_COLORS['status_error'])
            return False
        ...
        files_copied = 0
        if chapter_id is not None:
            chapter_folder_name = {-1: 'demo',
                                   0: 'chapter_0'}.get(chapter_id, f'chapter_{chapter_id}')
        ...
        if not mod_source_dir or not os.path.isdir(mod_source_dir):
            self.update_status_signal.emit(
                tr('status.no_files_to_copy'), UI_COLORS['status_warning'])
            return True
        if not hasattr(self, '_backup_temp_dir') or not self._backup_temp_dir:
            ...
            self._update_session_manifest(
                backup_temp_dir=self._backup_temp_dir)
        print(
            f'[XDELTA-DEBUG] _create_backup_and_copy_mod_files: chapter_id={chapter_id}, mod_source_dir={mod_source_dir}, target_dir={target_dir}, is_xdelta_mod={is_xdelta_mod}')
        for root, _, files in os.walk(mod_source_dir):
            for file in files:
                ...
                is_core_data_file = file_lower in ('data.win', 'data.ios', 'game.ios') or (file_lower.endswith('.win') and 'data' in file_lower) or (
                    file_lower.endswith('.ios') and 'game' in file_lower) or (is_xdelta_mod and file_lower.endswith('.xdelta'))
                if platform.system() == 'Darwin':
                    if is_core_data_file:
                        target_rel_path = os.path.join(
                            os.path.dirname(rel_path), 'game.ios')
                    elif file_lower.endswith('.win'):
                        name_without_ext = os.path.splitext(file)[0]
                        target_rel_path = os.path.join(
                            os.path.dirname(rel_path), name_without_ext + '.ios')
                elif is_core_data_file:
                    target_rel_path = os.path.join(
                        os.path.dirname(rel_path), 'data.win')
                elif file_lower.endswith('.ios'):
                    name_without_ext = os.path.splitext(file)[0]
                    target_rel_path = os.path.join(
                        os.path.dirname(rel_path), name_without_ext + '.win')
                ...
                try:
                    ...
                    if is_xdelta_mod and file_lower.endswith('.xdelta') and is_core_data_file:
                        if applied_xdelta_for_this_chapter:
                            continue
                        print(
                            f"[XDELTA-DEBUG] Chapter {chapter_id}: applying xdelta '{cache_file_path}' -> original in '{target_dir}' (computed game_file_path={game_file_path})")
                        if not self._apply_xdelta_patch(cache_file_path, game_file_path, target_dir):
                            self.update_status_signal.emit(
                                tr('errors.xdelta_apply_error', file=file), UI_COLORS['status_error'])
                            return False
                        files_copied += 1
                    ...
                    if file_lower.endswith(('.zip', '.rar', '.7z')) and (not is_core_data_file):
                        ...
                        self._update_session_manifest(
                            mod_files=[game_file_path])
                except Exception as e:
                    self.update_status_signal.emit(
                        tr('errors.file_copy_error', file=file, error=str(e)), UI_COLORS['status_error'])
        if files_copied > 0:
            self.update_status_signal.emit(
                tr('status.files_copied_count', count=files_copied), UI_COLORS['status_info'])
        else:
            self.update_status_signal.emit(
                tr('status.no_files_to_copy'), UI_COLORS['status_warning'])
        return True
    ...
    def _apply_xdelta_patch(self, xdelta_file_path: str, target_game_file_path: str, target_dir: str) -> bool:
        ...
        if not xdelta_exe:
            QMessageBox.critical(self, tr('errors.xdelta_error'), tr(
                'errors.xdelta_not_found', path='xdelta'))
            return False
        ...
        if not original_data_file:
            QMessageBox.critical(self, tr('errors.xdelta_error'), tr(
                'errors.original_data_file_not_found', target_dir=target_dir))
            return False
        if not hasattr(self, '_backup_files'):
            self._backup_files = {}
        if original_data_file not in self._backup_files:
            if not hasattr(self, '_backup_temp_dir') or not self._backup_temp_dir:
                self._backup_temp_dir = tempfile.mkdtemp(
                    prefix='deltahub_backup_')
                self._update_session_manifest(
                    backup_temp_dir=self._backup_temp_dir)
            unique_hash = hashlib.md5(
                original_data_file.encode('utf-8')).hexdigest()
            ...
            self._update_session_manifest(
                backup_files={original_data_file: backup_file_path})
        command = ['-d', '-f', '-s', self._backup_files[original_data_file],
                   xdelta_file_path, original_data_file]
        try:
            ...
            process = subprocess.run(command_to_run, capture_output=True, text=True,
                                     check=False, startupinfo=startupinfo, encoding='utf-8', errors='replace')
            if process.returncode == 0:
                self._mod_files_to_cleanup.append(original_data_file)
                self.update_status_signal.emit(tr('status.xdelta_patch_applied', patch_name=os.path.basename(
                    xdelta_file_path)), UI_COLORS['status_success'])
                return True
            else:
                shutil.copy2(
                    self._backup_files[original_data_file], original_data_file)
                error_message = process.stderr.strip() or process.stdout.strip()
                logging.error(f'Xdelta error: {error_message}')
                QMessageBox.critical(self, tr('errors.xdelta_patch_failed'), tr(
                    'errors.patch_incompatible_details', error=error_message))
                return False
        except FileNotFoundError:
            QMessageBox.critical(self, tr('errors.xdelta_error'), tr(
                'errors.xdelta_not_found', path=xdelta_exe))
            return False
        except Exception as e:
            QMessageBox.critical(self, tr('errors.xdelta_critical_error'), tr(
                'errors.xdelta_patch_critical_error', error=str(e)))
            try:
                shutil.copy2(
                    self._backup_files[original_data_file], original_data_file)
    ...
    def _extract_archive_to_target(self, archive_path: str, target_dir: str):
        ...
        try:
                    ...
                    for file in files:
                        ...
                        if platform.system() == 'Darwin':
                            if file_lower.endswith('.win'):
                                ...
                                target_file = os.path.join(os.path.dirname(
                                    target_file), name_without_ext + '.ios')
                        elif file_lower.endswith('.ios'):
                            name_without_ext = os.path.splitext(file)[0]
                            target_file = os.path.join(os.path.dirname(
                                target_file), name_without_ext + '.win')
                        ...
                        try:
                            if not hasattr(self, '_mod_dirs_to_cleanup'):
                                self._mod_dirs_to_cleanup = []
                            if target_dirname not in self._mod_dirs_to_cleanup:
                                self._mod_dirs_to_cleanup.append(
                                    target_dirname)
                                self._update_session_manifest(
                                    mod_dirs=[target_dirname])
                        except Exception:
                        ...
                        if os.path.exists(target_file):
                            ...
                            if hasattr(self, '_backup_temp_dir') and self._backup_temp_dir:
                                backup_file_path = os.path.join(
                                    self._backup_temp_dir, backup_rel_path)
                                os.makedirs(os.path.dirname(
                                    backup_file_path), exist_ok=True)
            ...
            self.update_status_signal.emit(tr('errors.archive_unpack_error', archive_name=os.path.basename(
                archive_path), error=str(e)), UI_COLORS['status_error'])
    ...
    def _determine_launch_config(self, selections: Dict[int, str]) -> Optional[Dict[str, Any]]:
        ...
        direct_launch_slot_id = self.local_config.get(
            'direct_launch_slot_id', -1)
        direct_launch = direct_launch_slot_id > 0 and self.game_mode.direct_launch_allowed and (
    ...
    def _handle_direct_launch(self, selected_tab_index: int) -> Optional[Dict[str, Any]]:
        chapter_folder = self._get_target_dir(
            self.game_mode.get_chapter_id(selected_tab_index))
        ...
        try:
            ...
            if use_custom_exe:
                target_exe = os.path.join(
                    chapter_folder, os.path.basename(source_exe))
            else:
                exe_name = 'UNDERTALE.exe' if isinstance(
                    self.game_mode, UndertaleGameMode) else 'DELTARUNE.exe'
                target_exe = os.path.join(chapter_folder, exe_name)
            shutil.copy2(source_exe, target_exe)
            self._direct_launch_cleanup_info = {
                'target_exe': target_exe, 'source_exe': source_exe, 'chapter_folder': chapter_folder, 'use_custom_exe': use_custom_exe}
            self._update_session_manifest(
                direct_launch=self._direct_launch_cleanup_info)
    ...
    def _cleanup_direct_launch_files(self):
        try:
            backed_up_targets = set(self._backup_files.keys()) if hasattr(
                self, '_backup_files') and self._backup_files else set()
            ...
            try:
                ...
                if hasattr(self, '_mod_dirs_to_cleanup') and self._mod_dirs_to_cleanup:
                    dirs = sorted(set(self._mod_dirs_to_cleanup), key=lambda p: len(
                        p.split(os.sep)), reverse=True)
                else:
                    data = self._load_session_manifest() or {}
                    dirs = sorted(set(data.get('mod_dirs_to_cleanup', [])), key=lambda p: len(
                        p.split(os.sep)), reverse=True)
                for d in dirs:
                    try:
            ...
            self.update_status_signal.emit(
                tr('errors.files_restore_error', error=str(e)), UI_COLORS['status_error'])
    ...
    def _launch_game_with_selections(self, selections: Dict[int, str]):
        ...
        if not (launch_config := self._determine_launch_config(selections)):
            ...
            return
        self.update_status_signal.emit(
            tr('status.launching_game'), UI_COLORS['status_success'])
    ...
    def _execute_game(self, launch_config: Dict[str, Any], vanilla_mode: bool = False):
        ...
        if not target_path:
            self.update_status_signal.emit(
                tr('errors.launch_target_not_defined'), 'red')
            self.restore_window_signal.emit()
            return
        try:
            if launch_type == 'webbrowser':
                self.monitor_thread = GameMonitorThread(
                    None, vanilla_mode, self)
                self.monitor_thread.finished.connect(
                    self._on_game_process_finished)
                self.monitor_thread.start()
                webbrowser.open(target_path)
                self.update_status_signal.emit(
                    tr('status.launching_via_steam'), UI_COLORS['status_steam'])
                return
            if not working_directory or not os.path.isdir(working_directory):
                msg = tr('errors.working_directory_not_found',
            ...
            system = platform.system()
            if system == 'Darwin':
                use_custom_exe = self.local_config.get(
                    'use_custom_executable', False)
                if use_custom_exe:
                    subprocess.Popen(['open', target_path])
                    self.update_status_signal.emit(
                        tr('status.macos_file_opened'), UI_COLORS['status_steam'])
                    if self.is_shortcut_launch:
                        sys.exit(0)
                    else:
                        QTimer.singleShot(
                            2000, self.restore_window_signal.emit)
                    return
            ...
            self.update_status_signal.emit(
                tr('status.game_launched_waiting_for_exit'), UI_COLORS['status_steam'])
            self.monitor_thread = GameMonitorThread(
                process, vanilla_mode, self)
            self.monitor_thread.finished.connect(
                self._on_game_process_finished)
            self.monitor_thread.start()
        except Exception as e:
            ...
            self.error_signal.emit(
                tr('errors.game_launch_failed', error=str(e)))
            self.restore_window_signal.emit()
    ...
    def _check_game_running(self, vanilla_mode):
        if is_game_running():
            QTimer.singleShot(
                2000, lambda: self._check_game_running(vanilla_mode))
        else:
            self.update_status_signal.emit(
    ...
    def _run_presence_tick(self):
        ...
        self.presence_thread.finished.connect(
            lambda: setattr(self, 'presence_thread', None))
        ...
        self.presence_worker.update_online_count.connect(
    ...
    def _update_online_label(self, count: int):
        if not self.is_shortcut_launch:
            self.online_label.setText(
                f"<span style='color:{UI_COLORS['status_ready']};'>●</span> {tr('status.online_count', count=count)}")
    ...
    def _update_custom_executable_ui(self):
        ...
        path = self.local_config.get(
            self.game_mode.get_custom_exec_config_key(), '')
        self.custom_exe_frame.setVisible(
            use_custom and self.use_custom_executable_checkbox.isEnabled())
        if self.custom_exe_frame.isVisible():
            self.custom_executable_path_label.setText(
                tr('ui.currently_selected', filename=os.path.basename(path)) if path else tr('ui.file_not_selected'))
    ...
    def _retranslate_texts(self):
        ...
        self.settings_title_label.setText(
            f"<h1>{tr('ui.settings_title')}</h1>")
        ...
        self.startup_sound_button.setText(
            self._get_startup_sound_button_text())
        self.disable_background_checkbox.setText(
            tr('checkboxes.disable_background'))
        ...
        self.changelog_button.setText(
            tr('buttons.changelog_close') if self.is_changelog_view else tr('buttons.changelog'))
        self.help_button.setText(
            tr('buttons.help_close') if self.is_help_view else tr('buttons.help'))
    ...
    def _update_mods_in_active_slots(self):
        ...
        if mods_to_update:
            self.pending_updates = mods_to_update[1:] if len(
                mods_to_update) > 1 else []
    ...
    def _on_xdelta_patch_click(self):
        try:
            ...
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.patching_window_failed', error=str(e)))
    ...
    def _show_main_mod_management_dialog(self):
        ...
        create_button.clicked.connect(
            lambda: self._on_create_mod_choice(dialog, has_internet))
        edit_button = QPushButton(tr('ui.edit_mod'))
        edit_button.setFixedSize(180, 50)
        edit_button.clicked.connect(
            lambda: self._on_edit_mod_choice(dialog, has_internet))
    ...
    def _on_create_mod_choice(self, parent_dialog, has_internet):
        ...
        public_button.clicked.connect(
            lambda: self._create_mod(dialog, public=True))
        ...
        local_button.clicked.connect(
            lambda: self._create_mod(dialog, public=False))
    ...
    def _create_mod(self, parent_dialog, public: bool):
        ...
        if public and (not check_internet_connection()):
            QMessageBox.critical(self, tr('errors.no_internet'), tr(
                'errors.public_mod_internet'))
            return
    ...
    def _edit_public_mod(self, parent_dialog):
        ...
        if not check_internet_connection():
            ...
            return
        secret_key, ok = QInputDialog.getText(self, tr('dialogs.enter_secret_key'), tr(
            'dialogs.secret_key_mod'), QLineEdit.EchoMode.Password)
        ...
        try:
            ...
            for h in candidate_hashes:
                resp = requests.get(
                    f'{CLOUD_FUNCTIONS_BASE_URL}/getModData?modId={h}', timeout=10)
                if resp.status_code == 200 and resp.json():
                    mod_data = resp.json()
                    found_hash = h
                    break
                resp = requests.get(
                    f'{CLOUD_FUNCTIONS_BASE_URL}/getPendingModData?modId={h}', timeout=10)
                if resp.status_code == 200 and resp.json():
                    mod_data = resp.json()
                    found_hash = h
            ...
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.key_check_failed', error=str(e)))
            return
        if not mod_data:
            QMessageBox.warning(self, tr('errors.mod_not_found'), tr(
                'errors.secret_key_invalid'))
            return
        if mod_data.get('ban_status', False):
            ban_reason = mod_data.get(
                'ban_reason', tr('defaults.not_specified_fem'))
            QMessageBox.critical(self, tr('dialogs.mod_blocked_title'), tr(
                'dialogs.mod_blocked_message', ban_reason=ban_reason, error_message=tr('dialogs.error_occurred')))
            return
        if found_in_pending:
            msg_box = QMessageBox(self)
            msg_box.setWindowTitle(tr('dialogs.mod_on_moderation'))
            msg_box.setText(tr('dialogs.mod_on_moderation_message'))
            withdraw_btn = msg_box.addButton(
                tr('buttons.withdraw_request'), QMessageBox.ButtonRole.DestructiveRole)
            ok_btn = msg_box.addButton(
                tr('buttons.ok'), QMessageBox.ButtonRole.AcceptRole)
            msg_box.setDefaultButton(ok_btn)
            msg_box.exec()
            if msg_box.clickedButton() == withdraw_btn:
                try:
                    from config.constants import CLOUD_FUNCTIONS_BASE_URL
                    requests.post(f'{CLOUD_FUNCTIONS_BASE_URL}/withdrawPendingMod',
                                  json={'hashedKey': hashed_key}, timeout=10)
                    QMessageBox.information(
                        self, tr('dialogs.request_withdrawn'), tr('dialogs.withdrawal_success'))
                except Exception as e:
                    QMessageBox.critical(self, tr('errors.error'), tr(
                        'errors.request_revoke_failed', error=str(e)))
            return
        try:
            from config.constants import CLOUD_FUNCTIONS_BASE_URL
            pending_changes_response = requests.get(
                f'{CLOUD_FUNCTIONS_BASE_URL}/getPendingChangeData?modId={hashed_key}', timeout=10)
            if pending_changes_response.status_code == 200 and pending_changes_response.json():
                ...
                withdraw_button = msg_box.addButton(
                    tr('buttons.withdraw_request'), QMessageBox.ButtonRole.DestructiveRole)
                msg_box.exec()
                ...
                if reply == withdraw_button:
                    try:
                        ...
                        delete_response = requests.post(
                            f'{CLOUD_FUNCTIONS_BASE_URL}/withdrawPendingChange', json={'hashedKey': hashed_key}, timeout=10)
        ...
        editor = ModEditorDialog(
            self, is_creating=False, is_public=True, mod_data=mod_data)
        editor.exec()
        try:
            self.activateWindow()
            self.raise_()
    ...
    def _edit_local_mod(self, parent_dialog):
            ...
            for folder_name in os.listdir(self.mods_dir):
                ...
                try:
                    ...
                    if config_data and config_data.get('is_local_mod'):
                        mod_info = {'key': config_data.get('mod_key'), 'name': config_data.get(
                            'name', 'Неизвестный мод'), 'data': config_data, 'folder_path': folder_path}
                        local_mods.append(mod_info)
                except Exception:
                    continue
        if not local_mods:
            QMessageBox.information(self, tr('dialogs.no_local_mods_title'), tr(
                'dialogs.no_local_mods_message'))
            return
        mod_names = [mod_info['name'] for mod_info in local_mods]
        selected_name, ok = QInputDialog.getItem(
            self, tr('dialogs.select_mod'), tr('dialogs.local_mods'), mod_names, 0, False)
        if not ok:
            return
        selected_mod = None
        ...
        if not selected_mod:
            QMessageBox.warning(self, tr('errors.error'),
                                tr('errors.selected_mod_not_found'))
            return
        mod_data = selected_mod['data'].copy()
        mod_data['key'] = selected_mod['key']
        mod_data['folder_name'] = os.path.basename(
            selected_mod['folder_path']) if selected_mod.get('folder_path') else ''
        editor = ModEditorDialog(
            self, is_creating=False, is_public=False, mod_data=mod_data)
        editor.exec()
        try:
            self.activateWindow()
            self.raise_()
    ...
    def _get_mod_status_for_chapter(self, mod: ModInfo, chapter_id: int) -> str:
        ...
        for mod_folder in os.listdir(self.mods_dir):
            ...
            try:
                ...
                logging.warning(
                    f'Failed to parse local config {config_path}: {e}')
                continue
        return 'install'
    ...
    def _is_mod_installed(self, mod_key: str) -> bool:
        ...
        for mod_folder in os.listdir(self.mods_dir):
            config_path = os.path.join(
                self.mods_dir, mod_folder, 'config.json')
            if os.path.isfile(config_path):
                try:
                    config_data = self._read_json(config_path)
                    stored_key = config_data.get(
                        'mod_key') or config_data.get('key')
            ...
            try:
                ...
                if 'installed_date' in config_data or 'is_available_on_server' in config_data:
                    ...
                    if 'installed_date' in config_data:
                        mods_metadata[mod_key]['installed_date'] = config_data.pop(
                            'installed_date')
                    if 'is_available_on_server' in config_data:
                        mods_metadata[mod_key]['is_available_on_server'] = config_data.pop(
    ...
    def _migrate_config_if_needed(self):
        ...
        defaults = {'game_path': '', 'last_selected': {}, 'use_custom_executable': False, 'demo_game_path': '', 'launch_via_steam': False, 'direct_launch_slot_id': -1, 'demo_mode_enabled': False, 'chapter_mode_enabled': False, 'custom_background_path': '', 'custom_executable_path': '',
                    'background_disabled': False, 'custom_color_background': '', 'custom_color_button': '', 'custom_color_border': '', 'custom_color_button_hover': '', 'custom_color_text': '', 'mods_dir_path': '', 'custom_color_version_text': '', 'beta_updates_enabled': False}
    ...
    def _update_qt_translations(self, language_code):
        ...
        qt_translation = localization_manager.get_qt_translation_name(
            language_code)
    ...
    def _get_executable_path(self):
        use_custom_exe = self.local_config.get('use_custom_executable', False)
        if use_custom_exe:
            custom_path = self.local_config.get(
                self.game_mode.get_custom_exec_config_key(), '')
        ...
        if not self.is_shortcut_launch:
            self.update_status_signal.emit(
                tr('errors.executable_not_found_deltarune'), UI_COLORS['status_error'])
        return None
    ...
    def _gather_shortcut_settings(self) -> Optional[Dict[str, Any]]:
        ...
        is_chapter_mode = hasattr(
            self, 'chapter_mode_checkbox') and self.chapter_mode_checkbox.isChecked()
        is_undertale_mode = isinstance(self.game_mode, UndertaleGameMode)
        settings = {'launcher_version': LAUNCHER_VERSION, 'game_path': self.game_path, 'demo_game_path': self.demo_game_path, 'is_demo_mode': is_demo_mode, 'is_chapter_mode': is_chapter_mode, 'is_undertale_mode': is_undertale_mode, 'launch_via_steam': self.launch_via_steam_checkbox.isChecked(), 'use_custom_executable': self.use_custom_executable_checkbox.isChecked(
        ), 'custom_executable_path': self.local_config.get(FullGameMode().get_custom_exec_config_key(), ''), 'demo_custom_executable_path': self.local_config.get(DemoGameMode().get_custom_exec_config_key(), ''), 'direct_launch_slot_id': self.local_config.get('direct_launch_slot_id', -1), 'mods': {}}
        if is_demo_mode:
            demo_mod_key = None
            try:
                demo_slot = self.slots.get(-10) if hasattr(self,
                                                           'slots') else None
                if demo_slot and getattr(demo_slot, 'assigned_mod', None):
                    demo_mod_key = getattr(demo_slot.assigned_mod, 'key', None) or getattr(
                        demo_slot.assigned_mod, 'mod_key', None)
            except Exception:
                demo_mod_key = None
            ...
            try:
                undertale_slot = self.slots.get(-20) if hasattr(self,
                                                                'slots') else None
                if undertale_slot and getattr(undertale_slot, 'assigned_mod', None):
                    undertale_mod_key = getattr(undertale_slot.assigned_mod, 'key', None) or getattr(
                        undertale_slot.assigned_mod, 'mod_key', None)
            except Exception:
                undertale_mod_key = None
            ...
            for slot_frame in self.slots.values():
                ...
                if chapter_id >= 0:
                    mod_key = None
                    if slot_frame.assigned_mod:
                        mod_key = getattr(slot_frame.assigned_mod, 'key', None) or getattr(
                            slot_frame.assigned_mod, 'mod_key', None)
                    settings['mods'][str(chapter_id)] = mod_key
        else:
            universal_mod_key = None
            try:
                universal_slot = self.slots.get(-1) if hasattr(self,
                                                               'slots') else None
                if universal_slot and getattr(universal_slot, 'assigned_mod', None):
                    universal_mod_key = getattr(universal_slot.assigned_mod, 'key', None) or getattr(
                        universal_slot.assigned_mod, 'mod_key', None)
            except Exception:
                universal_mod_key = None
    ...
    def _apply_mod_by_key(self, mod_key: str):
        ...
        if not os.path.exists(mod_folder):
            mod_folder = os.path.join(
                self.mods_dir, mod_config.get('name', ''))
            if not os.path.exists(mod_folder):
                raise Exception(
                    tr('errors.mod_files_not_found_by_key', mod_key=mod_key))
        ...
        try:
            ...
            if use_custom_executable:
                ...
                if exec_path and os.path.exists(exec_path):
                    ...
                    raise Exception(
                        tr('errors.specified_executable_not_found'))
            else:
                if isinstance(self.game_mode, UndertaleGameMode):
                    possible_names = ['UNDERTALE.exe', 'undertale.exe']
                else:
                    possible_names = [
                        'DELTARUNE.exe', 'deltarune.exe', 'SURVEY_PROGRAM.exe', 'survey_program.exe']
    ...
    def _save_shortcut(self, settings: Dict[str, Any]):
        ...
        shortcut_path, _ = QFileDialog.getSaveFileName(self, tr(
            'dialogs.save_shortcut'), os.path.expanduser(f'~/{default_name}'), file_filter)
        ...
        try:
            ...
            QMessageBox.information(self, tr('dialogs.success'), tr(
                'dialogs.shortcut_created_successfully', path=shortcut_path))
        except Exception as e:
            self.update_status_signal.emit(
                tr('status.shortcut_creation_error', error=str(e)), UI_COLORS['status_error'])
            QMessageBox.critical(self, tr('errors.error'), tr(
                'errors.shortcut_creation_failed', error=str(e)))
    ...
    def _get_target_dir(self, chapter_id):
        ...
        try:
            ...
            self.update_status_signal.emit(
                tr('errors.chapter_folder_search_error', error=str(e)), UI_COLORS['status_error'])
            return None
    ...
    def _has_mods_with_data_files(self, selections: Dict[int, str]) -> bool:
        for ui_index, mod_key in selections.items():
            ...
            if mod_key.startswith('local_'):
                mod_config = self._get_mod_config_by_key(mod_key)
                if mod_config:
                    chapter_files = mod_config.get(
                        'files', {}).get(str(chapter_id), {})
    ...
    def _find_and_validate_game_path(self, selections: Optional[Dict[int, str]] = None, is_initial: bool = False):
        ...
        skip_data_check = bool(
            selections and self._has_mods_with_data_files(selections))
        if isinstance(self.game_mode, DemoGameMode):
            game_type = 'deltarune'
        elif isinstance(self.game_mode, UndertaleGameMode):
            game_type = 'undertale'
        ...
        if is_valid_game_path(path_from_config, skip_data_check, game_type):
            self.update_status_signal.emit(
                tr('status.game_path', path=path_from_config), UI_COLORS['status_info'])
            return True
        self.update_status_signal.emit(
        ...
        if autodetected_path and is_valid_game_path(autodetected_path, skip_data_check, game_type):
            ...
            self.update_status_signal.emit(
                tr('status.game_folder_found', path=autodetected_path), UI_COLORS['status_success'])
            self._write_local_config()
            return True
        if is_initial:
            self.update_status_signal.emit(
    ...
    def _init_session(self):
        try:
            import requests
            from config.constants import CLOUD_FUNCTIONS_BASE_URL
            requests.post(f'{CLOUD_FUNCTIONS_BASE_URL}/presenceHeartbeat',
                          json={'sessionId': self.session_id}, timeout=5)
        except Exception:
            pass
    ...
    def _prompt_for_game_path(self, is_initial=False):
        ...
        if is_initial:
            QMessageBox.information(self, tr('dialogs.path_not_found'), tr(
                'dialogs.game_path_instruction', message=message))
        if platform.system() == 'Darwin':
            path, _ = QFileDialog.getOpenFileName(
                self, title, '', 'Application bundle (*.app);;All files (*)')
            if not path:
        ...
        if path:
            ...
            if is_valid_game_path(corrected_path, False, game_type):
                ...
                self._write_local_config()
                self.update_status_signal.emit(
                    tr('status.game_path_set', path=corrected_path), UI_COLORS['status_success'])
                self._update_action_button_state()
                return True
            else:
                QMessageBox.warning(self, tr('dialogs.invalid_folder'), tr(
                    'dialogs.invalid_game_folder'))
        if is_initial:
            self._start_background_music()
            self.initialization_finished.emit()
            self.update_status_signal.emit(
    ...
    def _handle_first_launch_settings(self):
        ...
        self._write_local_config()
        try:
            self.initialization_finished.disconnect(
                self._handle_first_launch_settings)
        except TypeError:
            pass
    ...
    def _on_theme_button_click(self):
        ...
        import_btn = msg.addButton(
            tr('buttons.import'), QMessageBox.ButtonRole.AcceptRole)
        export_btn = msg.addButton(
            tr('buttons.export'), QMessageBox.ButtonRole.AcceptRole)
    ...
    def _export_theme(self):
        ...
        if not theme_file_path:
            return
        theme_settings = {'custom_color_background': self.local_config.get('custom_color_background', ''), 'custom_color_button': self.local_config.get('custom_color_button', ''), 'custom_color_border': self.local_config.get('custom_color_border', ''), 'custom_color_button_hover': self.local_config.get(
            'custom_color_button_hover', ''), 'custom_color_text': self.local_config.get('custom_color_text', ''), 'custom_color_version_text': self.local_config.get('custom_color_version_text', ''), 'background_disabled': self.local_config.get('background_disabled', False), 'disable_splash': self.local_config.get('disable_splash', False)}
        with zipfile.ZipFile(theme_file_path, 'w') as zipf:
            ...
            if bg_path and os.path.exists(bg_path):
                zipf.write(
                    bg_path, f'background{os.path.splitext(bg_path)[1]}')
            music_path = self._get_background_music_path()
            if music_path and os.path.exists(music_path):
                zipf.write(
                    music_path, f'background_music{os.path.splitext(music_path)[1]}')
            sound_path = self._get_startup_sound_path()
            if sound_path and os.path.exists(sound_path):
                zipf.write(
                    sound_path, f'startup_sound{os.path.splitext(sound_path)[1]}')
        QMessageBox.information(self, tr('dialogs.success'), tr(
    ...
    def _import_theme(self):
        ...
        try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    for filename in os.listdir(temp_dir):
                        ...
                        if filename.startswith('background.'):
                            ...
                            dest_path = os.path.join(
                                self.config_dir, f'custom_background{ext}')
                            ...
                            shutil.copy2(src_path, os.path.join(
                                self.config_dir, f'custom_background_music{os.path.splitext(filename)[1]}'))
                        elif filename.startswith('startup_sound.'):
                            shutil.copy2(src_path, os.path.join(
                                self.config_dir, f'custom_startup_sound{os.path.splitext(filename)[1]}'))
            self._write_local_config()
            self._load_custom_style_settings()
            self.disable_background_checkbox.setChecked(
                self.local_config.get('background_disabled', False))
            ...
            self.apply_theme()
            QMessageBox.information(self, tr('dialogs.success'), tr(
                'dialogs.theme_imported_success'))
        except Exception as e:
            QMessageBox.critical(self, tr('dialogs.error'), tr(
                'dialogs.theme_import_failed', error=str(e)))
        self.local_config['first_launch_splash_shown'] = True
        self.local_config['disable_splash'] = True
        self._write_local_config()
        try:
    ...
    def _save_slots_state(self):
        ...
        is_chapter_mode = hasattr(
            self, 'chapter_mode_checkbox') and self.chapter_mode_checkbox.isChecked()
        config_key = self._get_slots_config_key(
            self.game_mode, is_chapter_mode)
        slots_data = {}
            ...
            if slot_frame.assigned_mod:
                ...
                if mod_key:
                    slots_data[str(slot_id)] = {
                        'mod_key': mod_key, 'mod_name': slot_frame.assigned_mod.name}
        self.local_config[config_key] = slots_data
        self._write_local_config()
    ...
    def _load_slots_state(self, mode=None):
        ...
        if isinstance(self.game_mode, DemoGameMode):
            ...
            is_chapter_mode = getattr(
                self, 'chapter_mode_checkbox', None) and self.chapter_mode_checkbox.isChecked()
        ...
        for slot_id, slot_data in slots_data.items():
            ...
            if not mod_data:
                ...
                for installed_mod in installed_mods:
                    installed_mod_key = installed_mod.get('mod_key') or installed_mod.get(
                        'key') or installed_mod.get('name')
                    if installed_mod_key == mod_key:
                        mod_data = self._create_mod_object_from_info(
                            installed_mod)
                        break
            if mod_data:
                current_slot = self._find_mod_in_slots(mod_data)
                if not current_slot:
                    self._assign_mod_to_slot(
                        slot_frame, mod_data, save_state=False)
