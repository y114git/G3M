...
class PluginManager(QObject):
            ...
            if os.path.isdir(plugin_path) and os.path.isfile(main_py_path):
                try:
                    ...
                    if spec and spec.loader:
                        ...
                        hooks = {
                            'on_before_game_launch': getattr(plugin_module, 'on_before_game_launch', None),
                            'on_after_game_launch': getattr(plugin_module, 'on_after_game_launch', None),
                            'on_before_game_exit': getattr(plugin_module, 'on_before_game_exit', None),
                            'on_after_game_exit': getattr(plugin_module, 'on_after_game_exit', None)
                        }
                        ...
                        plugin_info = {
                            'name_key': plugin_display_name_key,
    ...
    def get_background_plugins(self) -> List[Dict[str, Any]]:
        return [p for p in self.app_state.plugins if any(
            callable(p.get(hook)) for hook in [
                'on_before_game_launch', 'on_after_game_launch',
                'on_before_game_exit', 'on_after_game_exit'
            ]
        )]
