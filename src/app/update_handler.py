...
class _GlobalSettingsWorker(QThread):
    ...
    def run(self):
        try:
            ...
            logging.warning(
                f"_reload_global_settings: Failed to reload global settings: {e}"
            )
...
def handle_update_info(app, update_info, retry_count=0):
    ...
    logging.info(
        f"_handle_update_info: retry_count={retry_count}, initialization_completed={init_completed}, is_shown_to_user={is_shown}, is_visible={is_visible}"
    )
    if init_completed and (is_shown or is_visible):
        logging.info(
            f"_handle_update_info: Conditions met, showing update prompt for version {update_info.get('version', 'unknown')}"
        )
        if is_visible and (not is_shown):
            app.app_state.is_shown_to_user = True
            logging.info(
                "_handle_update_info: Set app_state.is_shown_to_user=True because window is visible"
            )
        app.show_update_prompt.emit(update_info)
    elif retry_count < max_retries:
        logging.debug(
            f"_handle_update_info: Conditions not met, scheduling retry (retry {retry_count + 1}/{max_retries})"
        )
        QTimer.singleShot(
            500, lambda: handle_update_info(app, update_info, retry_count + 1)
        )
    else:
        logging.warning(
            f"Update dialog: conditions not met after max retries (init_completed={init_completed}, is_shown={is_shown}, is_visible={is_visible}), showing dialog anyway"
        )
        app.show_update_prompt.emit(update_info)
    ...
    def _on_finished(success, data):
        try:
            if success:
                ...
                logging.info(
                    "_reload_global_settings: Global settings reloaded successfully"
                )
            if callback:
                callback(success)
        finally:
...
def check_and_show_announce(app, retry_count=0, force_check=False):
    ...
    is_visible = app.isVisible() if hasattr(app, "isVisible") else False
    logging.info(
        f"_check_and_show_announce: retry_count={retry_count}, initialization_completed={init_completed}, is_shown_to_user={is_shown}, is_visible={is_visible}, force_check={force_check}"
    )
    if init_completed and (is_shown or is_visible or force_check):
        ...
        if announce_version == 0:
            logging.info(
                "_check_and_show_announce: Announce version is 0, announcements disabled"
            )
            return
        saved_version = app.app_state.local_config.get("announce_version", 0)
        if saved_version == -1:
            logging.info(
                "_check_and_show_announce: User has disabled announcements (version -1)"
            )
            return
        if announce_version != saved_version:
            announce_message = app._localized_value(
                announce, "message_ru", "message_en"
            )
            if not announce_message:
                logging.info(
                    "_check_and_show_announce: No message for current language"
                )
                save_announce(app, announce_version)
                return
            announce_link = announce.get("link", "")
            logging.info(
                f"_check_and_show_announce: Conditions met, showing announce dialog (version {announce_version}, saved {saved_version})"
            )
            if is_visible and (not is_shown):
                app.app_state.is_shown_to_user = True
                logging.info(
                    "_check_and_show_announce: Set app_state.is_shown_to_user=True because window is visible"
                )
            from ui.dialogs.announce_dialog import AnnounceDialog

            dialog = AnnounceDialog(announce_message, announce_link, app)
            dialog.accepted_with_ok.connect(
                lambda: save_announce(app, announce_version)
            )
            dialog.exec()
            app.app_state.pending_announce_check = False
        else:
            logging.info(
                f"_check_and_show_announce: Announce version {announce_version} matches saved version, skipping"
            )
    elif retry_count < max_retries:
        logging.debug(
            f"_check_and_show_announce: Conditions not met, scheduling retry (retry {retry_count + 1}/{max_retries})"
        )
        QTimer.singleShot(
            500, lambda: check_and_show_announce(app, retry_count + 1, force_check)
        )
    else:
        logging.warning(
            f"Announce dialog: conditions not met after max retries (init_completed={init_completed}, is_shown={is_shown}, is_visible={is_visible}), skipping announce"
        )
...
def prompt_for_update(app, update_info):
    ...
    if app.feedback_service.ask_question(
        ...
        if (
            hasattr(app.app_state, "pending_announce_check")
            and app.app_state.pending_announce_check
        ):
