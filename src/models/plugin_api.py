...
class PluginAPI(QObject):
    def __init__(self, app_state, app_window, plugin_id: str, parent=None) -> None:
        super().__init__(parent)
        self.app_state, self.app_window, self.plugin_id = (
            app_state,
            app_window,
            plugin_id,
        )
    ...
    def _get_settings_service(self):
        return getattr(self.app_state, "settings_service", None) or getattr(
            self.app_window, "settings_service", None
        )

    def _get_feedback_service(self):
        return getattr(self.app_state, "feedback_service", None) or getattr(
            self.app_window, "feedback_service", None
        )

    def _load_config(self):
        prefixed_config = self.app_state.local_config.get("plugin_configs", {}).get(
            self.plugin_id, {}
        )
        self._config = {
            (
                key[len(f"{self.plugin_id}.") :]
    ...
    def _save_config(self):
        self.app_state.local_config.setdefault("plugin_configs", {})[self.plugin_id] = {
            f"{self.plugin_id}.{k}": v for k, v in self._config.items()
        }
    ...
    def get_mods(self) -> list[Any]:
        ...
        return (
            list(self.app_state.all_mods)
            if hasattr(self.app_state, "all_mods") and self.app_state.all_mods
            else []
        )

    def get_mod_by_key(self, key: str) -> Any | None:
    ...
    def show_message(self, message_type: str, title: str, message: str):
        ...
        if feedback:
            ...
            logging.warning(
                f"Plugin {self.plugin_id}: Cannot show message - feedback_service not available"
            )

    def log(self, level: str, message: str):
        logging.log(
            getattr(logging, level.upper(), logging.INFO),
            f"[Plugin {self.plugin_id}] {message}",
        )
    ...
    def get_game_mode(self) -> str | None:
        ...
        return (
            game_mode.__class__.__name__
            if game_mode and hasattr(game_mode, "__class__")
            else None
        )
