...
from ui.common.dialog_theme import (
    build_dialog_theme_stylesheet, get_dialog_theme_values, get_dialog_text_color,
)
...
def _list_local_versions(mod_folder: str) -> List[Dict]:
    ...
    for fname in sorted(os.listdir(versions_dir)):
        ...
        try:
            ...
            size, mtime = 0, 0
        result.append({'name': os.path.splitext(fname)[0], 'filename': fname, 'path': fpath, 'size': size, 'mtime': mtime})
    return result
...
def _sanitize_version_name(name: str) -> str:
    return ''.join(c if c.isalnum() or c in ' _-.' else '_' for c in name).strip() or 'version'


def _convert_archive_to_version_zip(archive_path: str, mod_folder: str, version_name: str) -> bool:
    ...
    try:
        ...
        logger.error('mod_versions: extract failed: %s - %s', type(e).__name__, e, exc_info=True)
        shutil.rmtree(temp_dir, ignore_errors=True)
        return False
...
class _ModVersionWorker(QThread):
    ...
    def __init__(self, url: str, mod_folder: str, version_name: str, parent=None):
    ...
    def run(self):
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(delete=False, suffix=suffix, prefix='mv_dl_')
            received = 0
            ...
            if self._cancelled:
                self.finished.emit(False, 'cancelled')
                return
            ok = _convert_archive_to_version_zip(tmp_path, self._mod_folder, self._version_name)
...
class _VersionItemWidget(QFrame):
    ...
    def _build_ui(self):
        ...
        if self._info.get('mtime'):
            parts.append(time.strftime('%Y-%m-%d %H:%M', time.localtime(self._info['mtime'])))
...
class ModVersionsDialog(QDialog):
    ...
    def __init__(self, mod_folder: str, mod_data, app_state, parent=None):
        super().__init__(parent)
        self._mod_folder = mod_folder
        self._mod_data = mod_data
        self._app_state = app_state
        self._version_widgets: Dict[str, _VersionItemWidget] = {}
        self._worker: Optional[_ModVersionWorker] = None
    ...
    def _is_gamebanana_mod(self) -> bool:
        if hasattr(self._mod_data, 'is_gamebanana_mod') and callable(self._mod_data.is_gamebanana_mod):
            return self._mod_data.is_gamebanana_mod()
        key = self._mod_data.get('key') if isinstance(self._mod_data, dict) else getattr(self._mod_data, 'key', None)
        return bool(key and isinstance(key, str) and (key.startswith('gb_mod_') or key.startswith('gb_wip_')))
    ...
    def _ask_version_name(self, default: str = '') -> Optional[str]:
        name, ok = QInputDialog.getText(
            self, tr('mod_versions.name_prompt_title'),
            tr('mod_versions.name_prompt'), text=default,
        )
    ...
    def _import_from_path(self, archive_path: str):
        ...
        try:
            _convert_archive_to_version_zip(archive_path, self._mod_folder, version_name)
            self._populate()
        except Exception as e:
    ...
    def _start_url_worker(self, url: str, version_name: Optional[str] = None):
        if self._is_busy():
            return
        if not version_name:
            base = os.path.splitext(os.path.basename(url.split('?')[0]))[0] or 'download'
            version_name = self._ask_version_name(default=base)
            if not version_name:
                return
        self._set_busy(True)
        self._worker = _ModVersionWorker(url, self._mod_folder, version_name, parent=self)
    ...
    def _on_download_from_gb(self):
        if self._is_busy():
            return
        from utils.mod_utils import get_gamebanana_mod_id, get_gamebanana_item_type
        mod_id_str = get_gamebanana_mod_id(self._mod_data)
        if not mod_id_str:
            QMessageBox.warning(self, tr('errors.error'), tr('errors.invalid_gamebanana_mod_id'))
            return
        mod_id = int(mod_id_str)
        itemtype = get_gamebanana_item_type(self._mod_data)
        from adapters.gamebanana_adapter import GameBananaAPI
        api = GameBananaAPI()
        all_files = self._format_gb_files(api.get_mod_files(mod_id, itemtype=itemtype) or [])
        if not all_files:
            QMessageBox.information(self, tr('mod_versions.title'), tr('errors.mod_no_files', mod_name=self._get_mod_attr('name', '')))
            return
        from ui.dialogs.file_picker_dialog import GameBananaFilePickerDialog
        picker = GameBananaFilePickerDialog(self, all_files, self._get_mod_attr('name', ''), self._get_mod_attr('external_url'))
        ...
        if not download_url:
            ...
            return
        file_name = selected.get('name') or selected.get('_sFile') or f'file_{selected.get("id", "unknown")}'
    ...
    def _format_gb_files(raw_files: list) -> List[Dict]:
        ...
        for f in raw_files:
            ...
            download_url = f.get('_sDownloadUrl') or f'https://gamebanana.com/dl/{file_id}'
            formatted.append({
                'id': file_id, '_idRow': file_id, 'name': name, '_sFile': name,
                'download_url': download_url, '_sDownloadUrl': download_url,
                'version': f.get('_sVersion', ''), 'description': f.get('_sDescription', ''),
                'size_bytes': f.get('_nFilesize') or 0, 'timestamp': f.get('_tsDateAdded'),
                'download_count': f.get('_nDownloadCount') or 0,
                'md5': f.get('_sMd5Checksum'), 'analysis_state': f.get('_sAnalysisState'),
                'analysis_result': f.get('_sAnalysisResult'), 'av_state': f.get('_sAvState'),
    ...
    def _on_switch(self, version_info: Dict):
        reply = QMessageBox.question(
            self, tr('mod_versions.confirm_switch_title'),
        ...
        try:
            ...
            QMessageBox.information(self, tr('mod_versions.title'), tr('mod_versions.switch_success', name=version_info['name']))
        except Exception as e:
    ...
    def _on_delete(self, version_info: Dict):
        reply = QMessageBox.question(
            self, tr('mod_versions.confirm_delete_title'),
