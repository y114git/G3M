...
class AboutDialog(QDialog):
    ...
    def __init__(self, parent, app_state, on_report_bug: Callable[[], None] | None = None):
    ...
    def _current_language_name(self) -> str:
        ...
        return localization_service.get_available_languages().get(language_code, language_code.upper())
    ...
    def _init_ui(self):
        ...
        self.version_label, self.version_value = self._add_info_row(info_layout, tr('ui.about_version_label', version='').rstrip(), LAUNCHER_VERSION)
        self.language_label, self.language_value = self._add_info_row(info_layout, tr('ui.about_language_label', language='').rstrip(), self._current_language_name())
        self.os_label, self.os_value = self._add_info_row(info_layout, tr('ui.about_os_label'), self._current_os_name())
        self.python_label, self.python_value = self._add_info_row(info_layout, tr('ui.about_python_label'), self._current_python_version())
        self.data_folder_label, self.data_path_edit = self._add_info_row(info_layout, tr('ui.about_data_folder'), self.data_root)
        ...
        self.telegram_button.clicked.connect(lambda: self._open_url(SOCIAL_LINKS['telegram']))
        self.discord_button = QPushButton(tr('buttons.discord'))
        self.discord_button.clicked.connect(lambda: self._open_url(SOCIAL_LINKS['discord']))
