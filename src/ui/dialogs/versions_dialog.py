"""Non-modal Versions Manager dialog with per-game filtering."""
import logging
...
class _VersionRecordWidget(QFrame):
    ...
    def __init__(self, record: VersionRecord, manager, app_state, parent=None):
        super().__init__(parent)
        self._record = record
        self._manager = manager
        self._app_state = app_state
        self.setObjectName('versions_record')
    ...
    def _build_ui(self):
        ...
        self._name_label.setObjectName('versions_record_name')
        self._name_label.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        top.addWidget(self._name_label)
        self._status_label = QLabel()
        self._status_label.setObjectName('versions_record_status')
        ...
        self._info_label.setObjectName('versions_record_info')
        layout.addWidget(self._info_label)

        btn_row = QHBoxLayout()
        btn_row.setSpacing(6)
        btn_row.addStretch()
        self._cancel_btn = QPushButton(tr('versions.action_cancel'))
        self._cancel_btn.setObjectName('versions_btn_cancel')
        self._cancel_btn.clicked.connect(self._on_cancel)
        self._cancel_btn.setVisible(False)
        btn_row.addWidget(self._cancel_btn)
        self._apply_btn = QPushButton(tr('versions.action_apply'))
        self._apply_btn.setObjectName('versions_btn_apply')
        self._apply_btn.clicked.connect(self._on_apply)
        btn_row.addWidget(self._apply_btn)
        self._export_btn = QPushButton(tr('versions.action_export'))
        self._export_btn.setObjectName('versions_btn_export')
        self._export_btn.clicked.connect(self._on_export)
        btn_row.addWidget(self._export_btn)
        self._delete_btn = QPushButton(tr('versions.action_delete'))
        self._delete_btn.setObjectName('versions_btn_delete')
    ...
    def _refresh(self):
        ...
        if applying:
            self._status_label.setText(tr('versions.status_applying'))
        elif busy:
            self._status_label.setText(tr('versions.status_busy'))
        elif r.archive_exists:
            self._status_label.setText(tr('versions.status_ready'))
        else:
            self._status_label.setText(tr('versions.status_missing'))
        parts = []
        if r.created_at:
        ...
        if r.file_count:
            parts.append(tr('versions.file_count', count=r.file_count))
    ...
    def _on_apply(self):
        reply = QMessageBox.question(
            self.window(), tr('versions.confirm_apply_title'),
            tr('versions.confirm_apply_text', name=self._record.display_name),
    ...
    def _on_export(self):
        self._manager.export_version(self._record.archive_path, parent_widget=self.window())
        self._refresh()

    def _on_delete(self):
        reply = QMessageBox.question(
            self.window(), tr('versions.confirm_delete_title'),
            tr('versions.confirm_delete_text', name=self._record.display_name),
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
            return
    ...
    def update_record(self, record: VersionRecord):
        self._record = record
        self._refresh()

    def relocalize_ui(self):
        self._apply_btn.setText(tr('versions.action_apply'))
        self._export_btn.setText(tr('versions.action_export'))
        self._delete_btn.setText(tr('versions.action_delete'))
        self._cancel_btn.setText(tr('versions.action_cancel'))
        self._refresh()
...
class VersionsDialog(QDialog):
    ...
    def __init__(self, manager, app_state, initial_game: str = 'deltarune', parent=None):
        ...
        self.setWindowTitle(tr('versions.title'))
    ...
    def _build_ui(self, initial_game: str):
        ...
        header = QHBoxLayout()
        self._title_label = QLabel(tr('versions.title'))
        self._title_label.setObjectName('versions_title')
        ...
        self._close_btn.setObjectName('versions_close_btn')
        ...
        self._add_btn.setObjectName('versions_add_btn')
        self._add_btn.setToolTip(tr('versions.add_tooltip'))
        ...
        self._empty_label = QLabel(tr('versions.empty_list'))
        self._empty_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._empty_label.setObjectName('versions_empty')
    ...
    def _apply_theme(self):
        ...
        extra = f'''
            QFrame#versions_record {{
                background-color: {theme["button"]};
                border: 2px solid {theme["border"]};
                border-radius: {theme["button_radius"]}px;
            }}
            QLabel#versions_record_name {{
                font-weight: bold;
                font-size: 13px;
            }}
            QLabel#versions_record_status {{
                font-size: 12px;
                color: {theme["secondary_text"]};
            }}
            QLabel#versions_record_info {{
                font-size: 11px;
                color: {theme["secondary_text"]};
            }}
            QLabel#versions_title {{
                font-size: 16px;
            }}
            QPushButton#versions_add_btn {{
                border: 2px solid {theme["border"]};
                border-radius: {theme["button_radius"]}px;
                background-color: {theme["button"]};
                margin: 0px;
            ...
            QPushButton#versions_add_btn:hover {{
                background-color: {theme["button_hover"]};
            }}
            QLabel#versions_empty {{
                font-size: 13px;
                color: {theme["secondary_text"]};
            }}
            QProgressBar {{
    ...
    def closeEvent(self, event):
        ...
        self._manager.progress_updated.disconnect(self._on_progress)
        self._manager.operation_error.disconnect(self._on_error)
    ...
    def _add_record_widget(self, record: VersionRecord):
    ...
    def _on_record_added(self, record: VersionRecord):
    ...
    def _on_record_updated(self, record: VersionRecord):
    ...
    def _on_add_clicked(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle(tr('versions.add_tooltip'))
        dialog.setModal(True)
        layout = QVBoxLayout(dialog)
        btn_layout = QHBoxLayout()
        create_btn = QPushButton(tr('versions.action_create'))
        ...
        import_btn = QPushButton(tr('versions.action_import'))
    ...
    def _do_create(self, game_id: str):
        ...
        from ui.dialogs.create_version_dialog import CreateVersionDialog
    ...
    def _do_import(self, game_id: str):
        ...
        dialog = ImportDialog(self, feedback, 'versions', '*.zip')
        if dialog.exec() == QDialog.DialogCode.Accepted:
            if dialog.import_method == 'file' and dialog.selected_file:
                self._manager.import_version_from_file(game_id, dialog.selected_file)
            elif dialog.import_method == 'url' and dialog.selected_url:
                self._manager.import_version_from_url(game_id, dialog.selected_url)
    ...
    def relocalize_ui(self):
        self.setWindowTitle(tr('versions.title'))
        self._title_label.setText(tr('versions.title'))
        self._close_btn.setText(tr('common.close'))
        self._empty_label.setText(tr('versions.empty_list'))
        self._add_btn.setToolTip(tr('versions.add_tooltip'))
