...
class ReportBugDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.text_edit.setToolTip(tr("tooltips.report_bug_text"))
    ...
    def _add_file(self):
        ...
        if ext not in [
            ...
            QMessageBox.warning(
                self,
                tr("dialogs.report_bug_title"),
                tr("dialogs.report_bug_invalid_file"),
            )
            return
        ...
        if current_total + file_size > self.max_total_size:
            QMessageBox.warning(
                self,
                tr("dialogs.report_bug_title"),
                tr("dialogs.report_bug_file_too_large"),
            )
            return
    ...
    def _update_file_size(self):
        ...
        self.file_size_label.setText(
            tr("dialogs.report_bug_file_size", size=f"{size_mb:.2f} MB")
        )
    ...
    def _send_report(self):
        ...
        self.worker = ReportBugWorker(
            self.text_edit.toPlainText().strip(),
            self.attached_files,
            self.attach_logs_checkbox.isChecked(),
            self.app_state,
        )
    ...
    def _on_worker_finished(self, success: bool, message: str):
        ...
        if success:
            QMessageBox.information(
                self, tr("dialogs.report_bug_title"), tr("dialogs.report_bug_success")
            )
            self.accept()
        else:
            QMessageBox.critical(
                self,
                tr("dialogs.report_bug_title"),
                tr("dialogs.report_bug_error", error=message),
            )
