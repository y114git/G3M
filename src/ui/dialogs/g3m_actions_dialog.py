...
_DATA_PATCH_FILTER = 'Data / Patch files (*.win *.ios *.unx *.droid *.zip *.xdelta);;All Files (*)'
...
class _PatchTab(QWidget):
    def __init__(self, g3m, app_state, parent=None):
        super().__init__(parent)
    ...
    def _build_ui(self):
        ...
        self._action_combo.addItems([tr('g3m_actions.action_create'), tr('g3m_actions.action_apply')])
    ...
    def _on_run(self):
        if not self._g3m or not self._g3m.is_available():
            QMessageBox.warning(self, tr('g3m_actions.title'), tr('errors.g3mtool_not_available'))
            return
        orig, second, out = self._original_row.path(), self._second_row.path(), self._output_row.path()
...
class _MergeTab(QWidget):
    ...
    def _on_add(self):
        paths, _ = QFileDialog.getOpenFileNames(self, tr('ui.select_file'), '', _DATA_PATCH_FILTER)
        for p in paths:
    ...
    def _on_run(self):
        ...
        patches = [self._file_list.item(i).data(Qt.ItemDataRole.UserRole) for i in range(self._file_list.count())]
        if not orig or len(patches) < 2 or not out:
            QMessageBox.warning(self, tr('g3m_actions.title'), tr('g3m_actions.merge_need_files'))
            return
        self._run_btn.setEnabled(False)
        self._status_label.setText(tr('g3m_actions.running'))
        self._worker = _WorkerThread(
            self._g3m.merge_patches,
            (orig, patches, out, None, None, self._code_cb.isChecked(), self._props_cb.isChecked()),
        )
        self._worker.finished.connect(self._on_finished)
        self._worker.start()
...
class _DiffTab(QWidget):
    def __init__(self, g3m, app_state, parent_dialog=None):
    ...
    def _on_finished(self, rc, out, err):
        ...
        if md_file:
            ...
            dlg = DiffViewerDialog(md_file, self._app_state, self._parent_dialog or self)
...
class G3MActionsDialog(QDialog):
    ...
    def __init__(self, g3m_manager, app_state, parent=None):
        super().__init__(parent)
