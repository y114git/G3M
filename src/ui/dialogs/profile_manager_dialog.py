...
from ui.common.styling import clamp_border_radius, get_border_radius, build_button_style
...
class ProfileManagerDialog(QDialog):
    ...
    def __init__(self, profile_service, app_state, parent=None):
    ...
    def _build_item_widget(self, summary: dict, is_active: bool = False) -> QWidget:
        ...
        parts = [f'{game_name} ({summary["game_mod_count"]})', tr('profiles.total_active', count=summary['total_mod_count'])]
    ...
    def _prompt_and_execute(self, action: callable, title_key: str, default_text: str = ''):
        name, ok = QInputDialog.getText(self, tr(title_key), tr('profiles.enter_name'), text=default_text)
        if ok and name.strip() and name.strip() != default_text:
            if action(name.strip()):
                self._refresh_list()
            else:
                QMessageBox.warning(self, tr('profiles.manager_title'), tr('profiles.already_exists'))
    ...
    def _on_duplicate(self):
        if source := self._selected_name():
            self._prompt_and_execute(lambda n: self.profile_service.duplicate(source, n), 'profiles.duplicate', f'{source} Copy')

    def _on_rename(self):
        old_name = self._selected_name()
        if old_name and old_name != DEFAULT_PROFILE:
            self._prompt_and_execute(lambda n: self.profile_service.rename(old_name, n), 'profiles.rename', old_name)
    ...
    def _apply_theme(self):
        ...
        br = clamp_border_radius(get_border_radius(self.app_state.local_config), width=38, height=38, border_width=2)
        sq_btn_qss = f'''
            QPushButton {{
        ...
        '''
        for attr, icon_name in (('add_btn', 'add'), ('dup_btn', 'duplicate'), ('edit_btn', 'edit'), ('del_btn', 'delete')):
            btn = getattr(self, attr)
            btn.setIcon(colored_icon(icon_name, icon_color))
            btn.setStyleSheet(sq_btn_qss)
        use_qss = build_button_style('profileUseBtn', '#4CAF50', '#5cb85c', '#e8e9eb', theme['border'], width=90, height=32, font_size=13, border_radius=br)
