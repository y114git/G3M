...
class ImportDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.url_input.returnPressed.connect(self._import_from_url)
        self.url_input.setPlaceholderText(tr(url_placeholder_key))
        url_layout.addWidget(self.url_input)
        self.url_import_button = QPushButton(tr(from_url_button_key))
        self.url_import_button.setAutoDefault(False)
        self.url_import_button.setEnabled(False)
        self.url_input.textChanged.connect(
            lambda text: self.url_import_button.setEnabled(bool(text.strip()))
        )
