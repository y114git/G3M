...
class XdeltaDialog(QDialog):
        ...
        if not path_to_check and (not getattr(sys, 'frozen', False)):
            for exe_name in exe_names:
                dev_path = os.path.abspath(os.path.join(os.path.dirname(
                    __file__), '..', '..', 'resources', 'bin', exe_name))
        ...
        if path_to_check:
            if system != 'Windows':
                try:
                    ...
                    logging.warning(
                        f'Could not set executable permission on {path_to_check}: {e}')
    ...
    def init_ui(self):
        ...
        create_layout.addWidget(self._create_file_group(tr('ui.original_file_label'), tr(
            'ui.select_original_file'), get_file_filter('all_files'), self.original_create_edit))
        create_layout.addWidget(self._create_file_group(tr('ui.modified_file_label'), tr(
            'ui.select_modified_file'), get_file_filter('all_files'), self.modified_create_edit))
        create_layout.addWidget(self._create_save_file_group(tr('ui.save_patch_as'), tr(
            'ui.specify_patch_path'), get_file_filter('xdelta_files'), self.patch_output_edit))
        ...
        apply_layout.addWidget(self._create_file_group(tr('ui.original_file_label'), tr(
            'ui.select_original_for_patch'), get_file_filter('all_files'), self.original_apply_edit))
        apply_layout.addWidget(self._create_file_group(tr('ui.patch_file_label'), tr(
            'ui.select_patch_file'), get_file_filter('xdelta_files'), self.patch_apply_edit))
        apply_layout.addWidget(self._create_save_file_group(tr('ui.save_modified_as'), tr(
            'ui.specify_save_path'), get_file_filter('all_files'), self.output_apply_edit))
    ...
    def _create_file_group(self, label_text, button_text, file_filter, line_edit):
        group_box, button = create_file_group_universal(
            label_text, button_text, file_filter, line_edit, 'open')
    ...
    def _create_save_file_group(self, label_text, button_text, file_filter, line_edit):
        group_box, button = create_file_group_universal(
            label_text, button_text, file_filter, line_edit, 'save')
        if line_edit is getattr(self, 'output_apply_edit', None):
            button.clicked.connect(
                lambda: self._browse_save_output_file(line_edit, file_filter))
        else:
            button.clicked.connect(
                lambda: self._browse_save_file(line_edit, file_filter))
        return group_box
    ...
    def _browse_save_file(self, line_edit, file_filter, suggested_name=''):
        file_path, _ = QFileDialog.getSaveFileName(
            self, tr('ui.save_file'), suggested_name, file_filter)
        if file_path:
    ...
    def create_patch(self):
        ...
        if not xdelta_path:
            self._show_message(tr('dialogs.error'), tr(
                'errors.xdelta_unavailable'), QMessageBox.Icon.Critical)
            return
        ...
        if not all((original_file, modified_file, output_patch)):
            self._show_message(tr('dialogs.error'), tr(
                'ui.select_all_files'), QMessageBox.Icon.Warning)
            return
        if not os.path.exists(original_file):
            self._show_message(tr('dialogs.error'), tr(
                'ui.original_file_not_found', path=original_file), QMessageBox.Icon.Warning)
            return
        if not os.path.exists(modified_file):
            self._show_message(tr('dialogs.error'), tr(
                'ui.modified_file_not_found', path=modified_file), QMessageBox.Icon.Warning)
            return
        temp_dir = None
        try:
            ...
            command = [xdelta_path, '-e', '-s',
                       temp_original, temp_modified, temp_output]
            result = subprocess.run(
                command, capture_output=True, text=True, encoding='utf-8')
            if result.returncode == 0:
                shutil.move(temp_output, output_patch)
                self._show_message(tr('ui.success'), tr(
                    'ui.patch_success', path=output_patch))
            else:
                error_message = result.stderr or result.stdout
                logging.error(f'xdelta create patch failed: {error_message}')
                self._show_message(tr('errors.patch_create_error'), tr(
                    'errors.patch_create_failed_details', error=error_message), QMessageBox.Icon.Critical)
        except Exception as e:
            self._show_message(tr('errors.patch_create_error'), tr(
                'errors.patch_create_exception', error=str(e)), QMessageBox.Icon.Critical)
        finally:
    ...
    def apply_patch(self):
        ...
        if not os.path.exists(patch_file):
            self._show_message(tr('dialogs.error'), tr(
                'ui.patch__not_found', path=patch_file), QMessageBox.Icon.Warning)
            return
        temp_dir = None
        try:
            temp_dir = tempfile.mkdtemp(prefix='xdelta_temp_')
            temp_original = os.path.join(
                temp_dir, 'original_source_for_patch.bin')
            ...
            command = [xdelta_path, '-d', '-s',
                       temp_original, temp_patch, temp_output]
            result = subprocess.run(
                command, capture_output=True, text=True, encoding='utf-8')
            if result.returncode == 0:
                shutil.move(temp_output, output_file)
                self._show_message(tr('ui.success'), tr(
                    'ui.patch_apply_success', path=output_file))
            else:
                error_message = result.stderr or result.stdout
                logging.error(f'xdelta apply patch failed: {error_message}')
                self._show_message(tr('errors.patch_apply_error'), tr(
                    'errors.patch_apply_failed_details', error=error_message), QMessageBox.Icon.Critical)
        except Exception as e:
            self._show_message(tr('errors.patch_apply_error'), tr(
                'errors.patch_apply_exception', error=str(e)), QMessageBox.Icon.Critical)
        finally:
            if temp_dir and os.path.exists(temp_dir):
                try:
                    shutil.rmtree(temp_dir)
