    ...
    for raw in body.splitlines():
        ...
        if s.startswith("|") and s.endswith("|"):
            ...
            lines.append(
                "<tr>"
                + "".join(
                    f"<{tag}>{_inline(c, inline_code_background_color)}</{tag}>"
                    for c in cells
                )
                + "</tr>"
            )
        ...
        if s.startswith("> "):
            if in_list:
                ...
                in_list = False
            lines.append(
                f'<blockquote style="border-left:3px solid {quote_border_color};'
...
def _inline(text: str, inline_code_background_color: str) -> str:
    ...
    text = re.sub(
        r"`(.+?)`",
        f'<code style="background:{inline_code_background_color};padding:1px 4px;border-radius:3px">\\1</code>',
        text,
    )
    return text
...
class _CollapsibleSection(QWidget):
    ...
    def __init__(
        self, index: int, title: str, body_html: str, font_family: str, parent=None
    ) -> None:
...
class DiffViewerDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        code_background_color = rgba_from_color(
            get_theme_color(config, "background", "#000000"),
            alpha=76,
