...
class PluginWidget(QFrame):
    ...
    def __init__(self, plugin_info: dict, parent=None, parent_app=None) -> None:
    ...
    def _apply_metrics(self):
        ...
        version_spacing = (
            ...
            and self.version_container
            and self.version_container.layout()
            else None
        )
        ...
        if (
            hasattr(self, "version_container")
            and self.version_container
            and self.version_container.layout()
        ):
        ...
        if name_key:
            ...
            if name_text.startswith("[") and name_text.endswith("]"):
                ...
                if plugin_id and name_key.startswith(f"{plugin_id}."):
                    ...
                    plugin_tr = (
                        self.parent_app.lang_service.get_plugin_tr(plugin_id)
                        if self.parent_app and hasattr(self.parent_app, "lang_service")
                        else None
                    )
                    if plugin_tr:
    ...
    def _init_ui(self):
        ...
        if author:
            ...
            self.author_label_title = QLabel(
                tr("ui.author_label"), self.author_container
            )
        ...
        if installed_date:
            ...
            self.installed_label_title = QLabel(
                tr("ui.installed_label"), self.installed_container
            )
            self.installed_label_title.setObjectName("primaryText")
            installed_label_value = QLabel(
                f" {installed_date}", self.installed_container
            )
        ...
        self.description_label = QLabel(
            description_text or tr("ui.no_description"), self
        )
        ...
        self.toggle_button.clicked.connect(
            lambda: self.toggle_requested.emit(self.plugin_name)
        )
        ...
        self.delete_button.setStyleSheet(
            build_button_style(
                "cardButton", "#F44336", "#da190b", text_color, border, border_radius=br
            )
        )
        self.delete_button.clicked.connect(
            lambda: self.delete_requested.emit(self.plugin_name)
        )
    ...
    def _update_status_indicator(self):
        ...
        color, tooltip_key = PLUGIN_STATUS_STYLES.get(
            status, ("#F44336", "plugins.status_broken")
        )
    ...
    def _update_toggle_button(self):
        ...
        if status == "enabled":
            ...
            apply_stylesheet_if_changed(
                ...
                cache_attr="_toggle_button_stylesheet_cache",
            )
        else:
    ...
    def _update_style(self):
        ...
        if hasattr(self, "icon_label"):
            apply_stylesheet_if_changed(
                ...
                cache_attr="_plugin_icon_label_stylesheet_cache",
            )
            self._load_icon()
        ...
        if hasattr(self, "name_label") and self.name_label:
            apply_stylesheet_if_changed(
                ...
                cache_attr="_plugin_name_stylesheet_cache",
            )
        ...
        if hasattr(self, "delete_button") and self.delete_button:
            ...
            apply_stylesheet_if_changed(
                ...
                cache_attr="_delete_button_stylesheet_cache",
            )
        self._update_toggle_button()
        self._update_status_indicator()
    ...
    def update_plugin_info(self, plugin_info: dict):
        ...
        if hasattr(self, "version_label") and self.version_label:
            ...
            if version_container_layout and isinstance(
                ...
                for i in range(layout.count()):
                    ...
                    if item and item.layout():
                        ...
                        if isinstance(metadata_layout, QHBoxLayout):
                            ...
                            self.author_label_title.setObjectName("primaryText")
                            self.author_label_value = QLabel(
                                f" {author}", self.author_container
                            )
                            self.author_label_value.setObjectName("secondaryText")
                            author_container_layout.addWidget(self.author_label_title)
                            author_container_layout.addWidget(self.author_label_value)
                            if (
                                hasattr(self, "installed_container")
                                and self.installed_container
                            ):
                                metadata_layout.insertWidget(
                                    metadata_layout.indexOf(self.installed_container),
    ...
    def relocalize_texts(self):
        ...
        self._update_toggle_button()
        self._update_status_indicator()
        if hasattr(self, "description_label") and not self._get_description_text():
            self.description_label.setText(tr("ui.no_description"))
    ...
    def _load_icon(self):
        ...
        if icon_path and os.path.exists(icon_path):
            try:
                pixmap = QPixmap()
                if pixmap.load(icon_path) and not pixmap.isNull():
                    icon_size = min(pixmap.width(), pixmap.height())
                    if icon_size > 0:
                        cropped = pixmap.copy(
                            (pixmap.width() - icon_size) // 2,
                            (pixmap.height() - icon_size) // 2,
                            icon_size,
                            icon_size,
                        )
                    ...
                    self.icon_label.setPixmap(
                        round_pixmap(scaled_pixmap, br, bw, border_color)
                        if (br > 0 or bw > 0)
                        else scaled_pixmap
                    )
                    self.icon_label.setText("")
        ...
        try:
            ...
            self.icon_label.setPixmap(
                round_pixmap(default_pixmap, br, bw, border_color)
                if (br > 0 or bw > 0)
                else default_pixmap
            )
            fallback_emoji = "🔌"
            ...
            apply_stylesheet_if_changed(
                self.icon_label,
                f"font-size: {max(36, round(48 * self._layout_scale()))}px; border: none; background: transparent;",
                cache_attr="_plugin_icon_label_stylesheet_cache",
            )
        except Exception:
            fallback_emoji = "🔌"
