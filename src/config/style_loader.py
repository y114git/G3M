...
from config.constants import ARROW_DOWN_SVG_TEMPLATE, ARROW_UP_SVG_TEMPLATE, STYLES_TEMPLATE_SUBDIR
...
def _write_arrow_svgs(icons_dir: str, text_color: str) -> tuple[str, str]:
    ...
    for template, path in [(ARROW_DOWN_SVG_TEMPLATE, down_path), (ARROW_UP_SVG_TEMPLATE, up_path)]:
...
def _ensure_arrow_svgs(text_color: str) -> tuple[str, str]:
    ...
    try:
        ...
        logger.exception('Failed to write temporary arrow SVGs; falling back to data URIs')
    import base64
    down_data = base64.b64encode(ARROW_DOWN_SVG_TEMPLATE.format(color=text_color).encode('utf-8')).decode('ascii')
    up_data = base64.b64encode(ARROW_UP_SVG_TEMPLATE.format(color=text_color).encode('utf-8')).decode('ascii')
...
def build_stylesheet(
    ...
    title_bar_menu_button_radius = border_radius_px(border_radius_value, height=20, border_width=1)
    title_bar_window_button_radius = '__TITLE_BAR_WINDOW_BUTTON_RADIUS__'
    title_bar_popup_radius = border_radius_px(border_radius_value, height=28, border_width=2)
    ...
    main_stylesheet = _apply_template_replacements(main_template, {'%frame_bg_color%': frame_bg_color, '%button_color%': button_color, '%border_color%': border_color, '%button_hover_color%': button_hover_color, '%main_text_color%': main_text_color, '%font_family_main%': font_family_main, '%font_size_main%': str(font_size_main), '%font_size_small%': str(font_size_small), '%checkbox_checked_color%': checkbox_checked_color, '%tooltip_bg_color%': tooltip_bg_color, '%custom_border_radius%': custom_border_radius, '%title_bar_radius%': title_bar_radius, '%title_bar_menu_button_radius%': title_bar_menu_button_radius, '%title_bar_window_button_radius%': title_bar_window_button_radius, '%title_bar_popup_radius%': title_bar_popup_radius, '%title_bar_popup_item_radius%': title_bar_popup_item_radius, '%button_border_radius%': button_border_radius, '%add_localization_button_radius%': add_localization_button_radius, '%top_refresh_button_radius%': top_refresh_button_radius, '%field_border_radius%': field_border_radius, '%tab_border_radius%': tab_border_radius, '%editor_border_radius%': editor_border_radius, '%checkbox_indicator_radius%': checkbox_indicator_radius, '%arrow_down_path%': arrow_down_path, '%arrow_up_path%': arrow_up_path})
    ...
    combined = combined.replace('__TITLE_BAR_WINDOW_BUTTON_RADIUS__', scaled_title_bar_window_button_radius)
