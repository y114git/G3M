...
class MultiModMerger(QObject):
    ...
    def _merge_mods_for_chapter(self, chapter_id: int, mods_list: List[Any], progress_base: int = 0, total_chapters: int = 1) -> bool:
        ...
        if not self._backup_file(chapter_id, data_win_path):
            return False
        original_data_win = data_win_path
        ...
        for idx, mod_data in enumerate(mods_to_apply):
            ...
            try:
                ...
                xdelta_msg = f'Applying xdelta patches from {mod_name} ({idx + 1}/{mods_count})...'
            ...
            if data_patches:
                ...
                if not self._apply_data_patches(mod_data_win, data_patches):
        ...
        mods_to_export = [m for i, m in enumerate(mods_to_apply) if mods_count - i != 1]
        for idx, mod_data in enumerate(mods_to_export):
            ...
            if not mod_data_win or not os.path.exists(mod_data_win):
                continue
            logging.info(f'Exporting modified assets from mod {mod_number} ({mod_name}) using ExportModifiedOnly')
            ...
            if export_script:
                returncode, stdout, stderr = self.utmtcli.execute_with_scripts(mod_data_win, ['ExportModifiedOnly'], output_path=mod_data_win, cwd=merge_root)
                if returncode != 0:
                    logging.warning(f'ExportModifiedOnly failed for mod {mod_number}: {stderr[:500]}')
                else:
                    logging.info(f'Successfully exported modified assets from mod {mod_number}')
        base_mod_number = 1
        ...
        if base_mod_number in mod_patched_files:
            ...
            if mod_1_patched and os.path.exists(mod_1_patched):
                if mod_1_patched != base_data_win:
                    ...
                    logging.info(f'Applied mod 1 xdelta to base file')
        chapter_file = os.path.join(cache_running_dir, 'chapterNumber.txt')
        with open(chapter_file, 'w', encoding='utf-8') as f:
            f.write(chapter_str)
        ...
        for idx, (mod_number, mod_name, objects_dir) in enumerate(objects_dirs_to_import):
            if self._cancelled:
                return False
            import_step = idx / len(objects_dirs_to_import) * import_progress if objects_dirs_to_import else 0
            current_progress = progress_base + int(xdelta_progress + export_progress + import_step)
            try:
                import_msg = tr('status.importing_assets', mod=mod_name, current=idx + 1, total=len(objects_dirs_to_import))
            except BaseException:
                import_msg = f'Importing assets from {mod_name} ({idx + 1}/{len(objects_dirs_to_import)})...'
        ...
        try:
            shutil.copy2(base_data_win, data_win_path)
            logging.info(f'Copied merged data.win to {data_win_path}')
        except Exception as e:
        ...
        for mod_data in mods_to_apply:
            mod_source_dir = self._get_mod_source_dir(mod_data, chapter_id)
            if mod_source_dir:
                if not self._apply_file_overrides(mod_source_dir, target_dir):
    ...
    def _apply_data_patches(self, data_win_path: str, data_patches: List[str]) -> bool:
        ...
        for patch_path in data_patches:
            try:
                ...
                logging.error(f'xdelta patch timed out: {patch_path}')
                return False
            except Exception as e:
                logging.error(f'xdelta patch error: {e}')
                return False
        return True
    ...
    def _merge_two_data_win_files(self, base_file: str, other_file: str) -> bool:
        try:
    ...
    def _simple_utmtcli_merge(self, data_win_path: str, mod_source_dir: str) -> bool:
        try:
            returncode, stdout, stderr = self.utmtcli.execute_command(['load', data_win_path, '--output', data_win_path], cwd=mod_source_dir)
            return returncode == 0
        except Exception as e:
            logging.error(f'Simple UTMTCLI merge failed: {e}')
            return False
    ...
    def _apply_file_overrides(self, mod_source_dir: str, target_dir: str) -> bool:
        if not os.path.isdir(mod_source_dir):
            return True
        ...
        for root, dirs, files in os.walk(mod_source_dir):
            for file in files:
                ...
                if file_lower.endswith(archive_extensions):
                    logging.debug(f'Extracting archive: {file}')
