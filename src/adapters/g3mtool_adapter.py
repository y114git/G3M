...
class G3MToolManager:
        ...
        if os.path.exists(exe_path):
            if self.platform != 'windows':
                try:
                    ...
                    logging.warning(f'Could not set executable permission on {exe_path}: {e}')
    ...
    def apply_patch(
        ...
        cmd = [self.g3mtool_path, 'patch', 'apply', original_data_win, patch_path, output_path]
        if log_path:
            cmd.extend(['--log', log_path])
        return self._run(cmd)
    ...
    def xpatch_apply(
        ...
        cmd = [self.g3mtool_path, 'xpatch', 'apply', original_file, patch_path, output_path]
        return self._run(cmd)
