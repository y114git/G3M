...
class GameBananaAPI:
    ...
    def _map_mod_data(self, gb_data, game_name, is_wip=False):
        ...
        nsfw_text_values = [gb_data.get('_sContentRating'), gb_data.get('ContentRating'), gb_data.get('MaturityRating'), category]
        is_nsfw = any(bool(gb_data.get(field)) for field in ('_bIsNsfw', 'is_nsfw', 'IsNsfw', 'nsfw', 'adult', 'is_adult', 'has_adult_content', '_bHasContentRatings', 'has_content_ratings'))
        if not is_nsfw:
            is_nsfw = any(('nsfw' in str(value).casefold()) or ('adult' in str(value).casefold()) or ('18+' in str(value).casefold()) for value in nsfw_text_values if value)
        if not is_nsfw:
